...
const VALUES_EN = [
  { icon: Heart, title: "Personal attention", text: "Every client and every instruction is different. We listen carefully and shape our advice around your priorities." },
  { icon: Shield, title: "Trust and transparency", text: "We communicate openly about pricing, terms and risk. No hidden charges and no pressure." },
  { icon: Target, title: "Focused on the outcome", text: "We measure success by well-managed transactions and clients who would choose us again." },
  { icon: Award, title: "Professional standards", text: "We invest continuously in expertise, presentation, technology and a better client experience." },
];
...
const TEAM_EN = TEAM.map((member, index) => ({
  ...member,
  role: ["Managing Director & Lead Agent", "Legal Department", "Residential Property Agent", "Mortgage Adviser"][index],
  bio: [
    "Twenty years in real estate, specialising in prime Prague property and carefully selected investment opportunities.",
    "A lawyer specialising in property law, transaction documents and Land Registry matters.",
    "A residential specialist covering Prague and the surrounding area, with a deliberately personal approach.",
    "A certified mortgage specialist who compares lenders and structures financing around the client.",
  ][index],
}));
...
const MILESTONES_EN = [
  { year: "2011", text: "Český Partner s.r.o. is established in central Prague." },
  { year: "2014", text: "Property management and lettings become dedicated areas of the business." },
  { year: "2017", text: "Our 500th successful sale and an eight-agent team." },
  { year: "2020", text: "Launch of virtual viewings, 3D presentations and a fully digital client journey." },
  { year: "2023", text: "More than 1,000 completed transactions and recognition for client service." },
  { year: "2026", text: "A new digital platform, dedicated investment division and broader regional coverage." },
];

export default function AboutPageContent({ locale = "cs" }: { locale?: SiteLocale }) {
  const en = locale === "en";
  const values = en ? VALUES_EN : VALUES;
  const team = en ? TEAM_EN : TEAM;
  const milestones = en ? MILESTONES_EN : MILESTONES;
  return (
    <>
      ...
      <main className="pt-16">
        ...
        <section className="bg-paper">
          <div className="mx-auto max-w-site px-6 pb-0 pt-10 xl:px-10">
            ...
            >
              ...
              <span className="text-ink">{en ? "About us" : "O nas"}</span>
            </nav>

            <Reveal className="mt-10 pb-20">
              <div className="grid items-end gap-12 lg:grid-cols-[1.2fr_1fr] lg:gap-20">
                <div>
                  <p className="eyebrow text-bronze-deep">{en ? "About us" : "O nas"}</p>
                  <h1 className="mt-4 text-[clamp(2.2rem,4.2vw,3.6rem)] font-semibold leading-[1.05] tracking-[-0.025em]">
                    {en ? "A property is more than an address —" : "Nemovitost je vic nez adresa —"}
                    <span className="text-bronze">
                      {" "}
                      {en ? " it is a decision that shapes your life." : " je to rozhodnuti na cely zivot."}
                    </span>
                  </h1>
                </div>
                <p className="max-w-md text-[15.5px] leading-[1.75] text-muted lg:pb-2">
                  {en
                    ? "From the first valuation to the handover of keys, we combine detailed knowledge of the Czech market with discreet, personal representation — whether you are selling a family home, looking for a new one or building an investment portfolio."
                    : "Od prvniho odhadu po predani klicu. Spojujeme detailni znalost ceskeho trhu s diskretnim, osobnim pristupem — at prodavate rodinny dum, hledate novy domov, nebo stavite investicni portfolio."}
                </p>
              </div>
            </Reveal>
          </div>
        ...
        <section className="bg-paper">
          <div className="mx-auto max-w-site px-6 py-24 md:py-28 xl:px-10">
            <Reveal>
              <p className="eyebrow text-muted">{en ? "Our values" : "Nase hodnoty"}</p>
              <h2 className="mt-4 text-[clamp(1.9rem,3vw,2.75rem)] font-semibold leading-[1.1] tracking-[-0.02em]">
                {en ? "What we stand for" : "Na cem stavime"}
              </h2>
            </Reveal>
        ...
        <section className="border-t border-line bg-stone/40">
          <div className="mx-auto max-w-site px-6 py-24 md:py-28 xl:px-10">
            <Reveal>
              <p className="eyebrow text-bronze-deep">{en ? "Our journey" : "Nase cesta"}</p>
              <h2 className="mt-4 text-[clamp(1.9rem,3vw,2.75rem)] font-semibold leading-[1.1] tracking-[-0.02em]">
                {en ? "Milestones that shaped us" : "Milniky, ktere nas formovaly"}
              </h2>
            </Reveal>
        ...
        <section className="bg-paper">
          <div className="mx-auto max-w-site px-6 py-24 md:py-28 xl:px-10">
            <Reveal>
              <p className="eyebrow text-muted">{en ? "Our team" : "Nas tym"}</p>
              <h2 className="mt-4 text-[clamp(1.9rem,3vw,2.75rem)] font-semibold leading-[1.1] tracking-[-0.02em]">
                {en ? "The people behind your transaction" : "Lide, kteri stoji za vasim obchodem"}
              </h2>
            </Reveal>
                    ...
                    <div className="p-7">
                      ...
                      <div className="mt-5 flex gap-3">
                        <a
                          ...
                          aria-label={`${en ? "Call" : "Zavolat"} ${t.name}`}
                        >
        ...
        <section className="bg-ink text-paper">
          <div className="mx-auto max-w-site px-6 py-24 md:py-28 xl:px-10">
            <div className="grid items-center gap-16 lg:grid-cols-2 lg:gap-24">
              <Reveal>
                <p className="eyebrow text-paper/50">{en ? "Our network" : "Spoluprace"}</p>
                <h2 className="mt-4 text-[clamp(1.9rem,3vw,2.75rem)] font-semibold leading-[1.1] tracking-[-0.02em]">
                  {en ? "Trusted specialists, working as one team" : "Pracujeme s temi nejlepsimi"}
                </h2>
                <p className="mt-5 max-w-md text-[15px] leading-[1.75] text-paper/60">
                  {en ? "We work with proven lawyers, banks, valuers and developers. Our professional network helps every transaction progress safely and smoothly." : "Spolupracujeme s proverenimy advokáty, bankami, znalci a developery. Nase sít partneru zajistuje, ze kazdy obchod probehne hladce a bezpecne."}
                </p>
              </Reveal>
              ...
              <Reveal delay={100}>
                <div className="grid grid-cols-2 gap-px bg-white/10">
                  {[
                    { icon: Landmark, label: en ? "Property lawyers" : "Advokátní kanceláře" },
                    { icon: Users, label: en ? "Mortgage specialists" : "Hypoteční specialisté" },
                    { icon: Shield, label: en ? "Property insurance" : "Pojištění nemovitostí" },
                    { icon: Award, label: en ? "Certified valuers" : "Certifikovaní odhadci" },
                  ].map((p) => (
                    <div
                      key={p.label}
        ...
        <section className="border-t border-line bg-paper">
          <div className="mx-auto max-w-site px-6 py-24 text-center md:py-32 xl:px-10">
            <Reveal>
              <p className="eyebrow text-bronze-deep">{en ? "Next step" : "Dalsi krok"}</p>
              <h2 className="mx-auto mt-4 max-w-2xl text-[clamp(1.9rem,3.4vw,3rem)] font-semibold leading-[1.1] tracking-[-0.02em]">
                {en ? "Shall we meet?" : "Chcete se s nami setkat?"}
              </h2>
              <p className="mx-auto mt-5 max-w-md text-[15px] leading-[1.75] text-muted">
                {en ? "Visit our office for a coffee, call us or write to us. Your first consultation is always complimentary." : "Stavte se k nam do kancelare na kavu, zavolejte nebo napiste. Prvni konzultace je vzdy zdarma."}
              </p>
