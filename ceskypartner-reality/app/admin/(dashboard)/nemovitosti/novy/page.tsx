...
type ImageItem = { id: string; url: string; file?: File; alt: string };

const DISPOSITIONS = ["1+kk", "1+1", "2+kk", "2+1", "3+kk", "3+1", "4+kk", "4+1", "5+kk", "5+1", "6+kk", "6+1"];
const PENB = ["A", "B", "C", "D", "E", "F", "G"];
const AMENITIES = [
...
const selectClass =
  "h-10 w-full cursor-pointer appearance-none rounded-xl border border-[var(--a-border)] bg-transparent px-4 pr-10 text-[13px] text-[var(--a-text)] outline-none transition-all duration-300 focus:border-[var(--a-bronze)]/30";
...
export default function NewListingPage() {
  ...
  async function handleGeocode() {
    ...
    if (!query) {
      alert("Vyplňte nejdřív adresu nebo lokalitu");
      return;
    }
    setGeocoding(true);
    try {
  ...
  async function handleSave() {
    ...
    for (const img of images) {
      if (img.file) {
        ...
        fd.append("watermark", "true"); // fotky inzerátu — vodoznak dle nastavení
        const res = await fetch("/api/admin/upload", { method: "POST", body: fd });
        if (res.ok) {
          const media = await res.json();
          uploadedImageUrls.push(media.url);
        }
      }
    }
    ...
    const body: any = {
      ...
      tourUrl: tourUrl.trim() || undefined,
      ...
      monthlyFees: monthlyFees ? Number(monthlyFees) : undefined,
      ...
      ruianAddressCode: ruianAddressCode || undefined,
      ruianParcelCode: ruianParcelCode || undefined,
    ...
    if (status === "ACTIVE") body.publishedAt = new Date().toISOString();

    const { data, error } = await apiPost("/api/admin/listings", body);
    setSaving(false);
    ...
    <div className="mx-auto max-w-[960px] space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[22px] font-semibold tracking-[-0.02em] text-[var(--a-text)]">Novy inzerat</h1>
            <p className="text-[12.5px] text-[var(--a-text-3)]">Vyplnte udaje o nemovitosti</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {saving ? <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[#0a0a0b] border-t-transparent" /> : <Save size={14} />}
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-semibold transition-all duration-300 ${
                  status === s
                    ? "border-[var(--a-bronze)]/30 " + info.bg
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-6">
          <SectionCard title="Zakladni udaje">
            <div className="space-y-4">
              <Field label="Nazev inzeratu" required>
                <input type="text" className={inputClass} placeholder="Napr. Penthouse s terasou, Vinohrady" value={title} onChange={(e) => setTitle(e.target.value)} />
              </Field>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <Field label="Cena (Kc)" required>
                  <input type="number" className={inputClass} placeholder="0" value={price} onChange={(e) => setPrice(e.target.value)} />
                </Field>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <Field label="Lokalita" required>
                  <input type="text" className={inputClass} placeholder="Praha 2 - Vinohrady" value={location} onChange={(e) => setLocation(e.target.value)} />
                </Field>
                <Field label="Adresa">
                  <input type="text" className={inputClass} placeholder="Ulice, cislo" value={address} onChange={(e) => setAddress(e.target.value)} />
                </Field>
                <Field label="PSC">
              ...
              <div className="grid items-end gap-4 sm:grid-cols-[1fr_1fr_auto]">
                <Field label="GPS lat">
                  <input type="number" step="0.0001" className={inputClass} placeholder="50.0755" value={lat} onChange={(e) => setLat(e.target.value)} />
                </Field>
                <Field label="GPS lng">
                  <input type="number" step="0.0001" className={inputClass} placeholder="14.4378" value={lng} onChange={(e) => setLng(e.target.value)} />
                </Field>
                <button
                  type="button"
              ...
              {lat && lng ? (
                ...
                <div className="flex h-48 items-center justify-center rounded-xl border border-dashed border-[var(--a-border)] bg-[var(--a-surface-2)]">
                  <div className="text-center">
                    <MapPin size={24} className="mx-auto text-[var(--a-text-3)]" />
                    <p className="mt-2 text-[12px] text-[var(--a-text-3)]">Zadejte GPS nebo pouzijte tlacitko &bdquo;Najit GPS z adresy&ldquo;</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <SectionCard title="Popis nemovitosti">
            <textarea
              ...
              placeholder="Detailni popis nemovitosti..."
              value={description}
          ...
          <SectionCard title="Fotogalerie">
            <div
              ...
              className={`flex flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-10 transition-all duration-300 ${
                dragOver ? "border-[var(--a-bronze)] bg-[var(--a-bronze-glow)]" : "border-[var(--a-border)] bg-[var(--a-surface-2)]"
              }`}
            >
              ...
              <p className="mt-3 text-[13px] text-[var(--a-text)]">
                ...
                <button
                  ...
                  className="font-semibold text-[var(--a-bronze)] underline-offset-2 hover:underline"
                >
                  vyberte ze souboru
                </button>
              </p>
              <p className="mt-1 text-[11px] text-[var(--a-text-3)]">
                JPG, PNG, WebP (auto konverze do WebP) &middot; max 10 MB
              </p>
              <input
                ref={fileRef}
                type="file"
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {images.map((img, i) => (
                  <div key={img.id} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-[var(--a-border)] bg-[var(--a-surface-2)]">
                    ...
                    <div className="absolute left-1.5 top-1.5 flex h-6 items-center gap-1 rounded bg-black/70 px-2 text-[10px] font-semibold text-white opacity-0 transition-opacity group-hover:opacity-100">
                      <GripVertical size={10} />
                      {i + 1}
                    </div>
                    {i === 0 && (
                      <span className="absolute bottom-1.5 left-1.5 rounded bg-[var(--a-bronze)] px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-[#0a0a0b]">
                        Hlavni
                      </span>
                    )}
                  </div>
        ...
        <div className="space-y-6">
          <SectionCard title="Parametry">
            <div className="space-y-4">
              <Field label="Uzitna plocha (m2)">
                <input type="number" className={inputClass} placeholder="0" value={area} onChange={(e) => setArea(e.target.value)} />
              </Field>
              <Field label="Plocha pozemku (m2)">
                <input type="number" className={inputClass} placeholder="0" value={landArea} onChange={(e) => setLandArea(e.target.value)} />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Podlazi">
                  <input type="number" className={inputClass} placeholder="0" value={floor} onChange={(e) => setFloor(e.target.value)} />
                </Field>
                <Field label="Pocet podlazi">
                  <input type="number" className={inputClass} placeholder="0" value={floors} onChange={(e) => setFloors(e.target.value)} />
                </Field>
              </div>
              <Field label="Rok vystavby">
                <input type="number" className={inputClass} placeholder="2024" value={yearBuilt} onChange={(e) => setYearBuilt(e.target.value)} />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Mesicni naklady (Kc)">
                  <input type="number" className={inputClass} placeholder="0" value={monthlyFees} onChange={(e) => setMonthlyFees(e.target.value)} />
                </Field>
                <Field label="Vratna kauce (Kc)">
                  <input type="number" className={inputClass} placeholder="0" value={deposit} onChange={(e) => setDeposit(e.target.value)} />
                </Field>
              </div>
                ...
                <div className="flex gap-1">
                  {PENB.map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setPenb(p)}
                      className={`flex h-8 w-8 items-center justify-center rounded-lg text-[11px] font-semibold transition-all duration-300 border ${
                        penb === p
          ...
          <SectionCard title={`Export na portaly (${selectedPortals.size})`}>
            ...
            <p className="mt-3 text-[11px] text-[var(--a-text-3)]">
              Po ulozeni se inzerat zaradi do fronty exportu. Stav sledujte v sekci Export portaly.
            </p>
          </SectionCard>
        </div>
      </div>
      ...
      >
        <div className="mx-auto flex max-w-[960px] items-center justify-between">
          <p className="text-[12px] text-[var(--a-text-3)]">Neulozone zmeny</p>
          <div className="flex gap-2">
            <Link href="/admin/nemovitosti" className="rounded-xl border border-[var(--a-border)] px-5 py-2.5 text-[12.5px] font-semibold text-[var(--a-text-2)] transition-all duration-300 hover:border-[var(--a-border-hover)] hover:text-[var(--a-text)]">
              Zahodit
            </Link>
            <button
              onClick={handleSave}
