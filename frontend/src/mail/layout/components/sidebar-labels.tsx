...
export function SidebarLabels() {
  ...
  const MENU_LABELS_CONFIG: MenuLabelConfig[] = [
    {
      title: language === "cs" ? "Štítky" : "Labels",
      children: [
        {
          title: language === "cs" ? "Archiv" : "Archive",
          path: '/core/mail/inbox?label=archive',
          count: 12,
          icon: Archive,
        },
        {
          title: language === "cs" ? "Odloženo" : "Snoozed",
          path: '/core/mail/inbox?label=snoozed',
          count: 4,
          icon: Clock,
        },
        {
          title: language === "cs" ? "Spam" : "Spam",
          path: '/core/mail/inbox?label=spam',
          count: 3,
          icon: AlertTriangle,
        },
        {
          title: language === "cs" ? "Koš" : "Trash",
          path: '/core/mail/inbox?label=trash',
          count: 34,
          icon: Trash,
        },
      ],
        ...
        return (
          <AccordionMenuGroup key={index}>
            ...
            {customLabels.map((child, index) => (
              <AccordionMenuItem key={`custom-${index}`} value={`/core/mail/inbox?label=custom-${index}`}>
                <Link to={`/core/mail/inbox?label=custom-${index}`} className="">
                  {child.icon && <child.icon />}
