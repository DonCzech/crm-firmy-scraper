	...
	useEffect(() => {
		...
		if (mode === 'reply') {
			...
			if (emailData && emailData.sender) {
				...
				setEmailChips([{
					id: '1',
					email: emailData.senderEmail || `noreply@${domain}.com`,
					logo: logoData.name,
...
const handleEmailInputKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
  if (e.key === 'Enter' && emailAddress.trim()) {
    ...
    if (findInvalidEmails(email).length > 0) {
      toast.custom(
        (t) => (
          <Alert variant="mono" icon="success" onClose={() => toast.dismiss(t)}>
            ...
            <AlertTitle>Invalid email address: {email}</AlertTitle>
          </Alert>
        ),
        { duration: 3000 },
      );
...
const generateEmailContent = async () => {
  ...
  try {
    const toRaw = [...emailChips.map((chip) => chip.email), emailAddress.trim()]
      .filter(Boolean)
      .join(', ');
...
const handleSend = async () => {
  ...
  const invalidTo = findInvalidEmails(toRaw);
  ...
  return (
		<>
		<div className="border bg-background mx-4 rounded-lg relative">
			...
			{/* Bottom toolbar */}
			<div className="sticky bottom-0 z-30 flex items-center justify-between p-4 bg-background border-t rounded-b-lg pointer-events-auto">
