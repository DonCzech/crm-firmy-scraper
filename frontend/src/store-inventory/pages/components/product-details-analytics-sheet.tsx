...
import type { BackendProduct } from '@/crm/services/backend';
...
export function ProductDetailsAnalyticsSheet({
  ...
  const galleryImages = useMemo(() => {
    const inputImages = (product?.images && product.images.length > 0 ? product.images : [product?.image]).filter(
      Boolean,
    ) as string[];
    const resolved = inputImages.map((item) => resolveProductImageSrc(item)).filter(Boolean) as string[];
    return resolved.length > 0 ? resolved : [toAbsoluteUrl('/media/store/client/1200x1200/11.png')];
  }, [product?.image, product?.images]);

  useEffect(() => {
    setSelectedImageSrc(galleryImages[0] || '');
  }, [galleryImages, open]);
          ...
          <div className="flex justify-between gap-2 border-b border-border px-5 py-4">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center flex-wrap gap-1.5 text-2sm">
                ...
                <span className="font-medium text-foreground/80">{productSku}</span>
              ...
              <div className="w-full shrink-0 lg:w-[420px] py-5 lg:ps-5">
                <div className="mb-5">
                  <Card className="flex items-center justify-center rounded-md bg-accent/50 shadow-none shrink-0 mb-5">
                    <img
                      src={selectedImageSrc || galleryImages[0]}
                  ...
                  <ToggleGroup
                    ...
                    onValueChange={(newValue) => {
                      if (newValue) setSelectedImageSrc(newValue);
                    }}
                  >
