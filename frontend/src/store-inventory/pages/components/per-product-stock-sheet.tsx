...
import { updateProduct } from '@/crm/services/backend';
...
export function PerProductStockSheet({
  ...
  useEffect(() => {
    if (!open) return;
    setCurrentStock(String(data?.stock ?? 0));
    setCategoryValue(data?.category || data?.trend?.label || 'General');
    ...
    setPriceValue(Number.isFinite(parsed) ? String(parsed) : '0');
  }, [open, data]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Změny produktu byly uloženy');
      onOpenChange(false);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Uložení produktu selhalo');
    } finally {
      setSaving(false);
    }
  };
  ...
  const items = useMemo<Item[]>(
    () => [
      ...
      { label: 'Category', info: categoryValue || 'General' },
      { label: 'Rating', info: '4' },
      { label: 'Price', info: `${priceValue || '0'} Kč` },
    ],
    [data, categoryValue, priceValue],
  );

  const imageSrc = useMemo(() => {
    const image = data?.productInfo?.image;
    ...
    if (image.startsWith('/media/')) return toAbsoluteUrl(image);
    return toAbsoluteUrl(`/media/store/client/1200x1200/${image}`);
      ...
      <SheetContent className="gap-0 lg:w-[960px] sm:max-w-none inset-5 border start-auto h-auto rounded-lg p-0 [&_[data-slot=sheet-close]]:top-4.5 [&_[data-slot=sheet-close]]:end-5">
            ...
            <div className="flex flex-wrap lg:flex-nowrap px-3.5 grow">
              <div className="grow lg:border-e border-border lg:pe-5">
                ...
                <div className="flex flex-col gap-2.5 py-5">
                  ...
                  <div className="grid md:grid-cols-2 gap-2.5">
                    ...
                    <div className="flex flex-col gap-2.5 grow">
                      <Label className="text-xs">Price (Kč)</Label>
              ...
              <div className="w-full shrink-0 lg:w-[320px] py-5 lg:ps-5">
                ...
                <span className="text-secondary-foreground text-2sm font-normal leading-4">
                  {data?.description || data?.productInfo?.title || 'Product detail'}
                </span>
        ...
        <SheetFooter className="flex items-center not-only-of-type:justify-between border-t py-5 px-5 border-border gap-2">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <Button variant="mono" onClick={() => void handleSave()} disabled={saving}>
              {saving ? 'Saving...' : 'Save'}
            </Button>
          </div>
        </SheetFooter>
