...
export function ProductFormImageUpload({ 
  ...
  const resolveProductImageSrc = useCallback((image?: string) => {
  ...
  useEffect(() => {
    ...
    const sourceImages = (initialImages && initialImages.length > 0 ? initialImages : [initialImage]).filter(
      Boolean,
    ) as string[];

    const resolved = sourceImages
      .map((item) => resolveProductImageSrc(item))
    ...
    setAllImages(
      resolved.map((src, index) => ({
        id: `default-current-${index}`,
        src,
        alt: `Current product image ${index + 1}`,
      })),
    );
  }, [isEditMode, initialImage, initialImages, resolveProductImageSrc]);
  ...
  const removeImage = useCallback((id: string) => {
    setAllImages((prev) => {
      ...
      const uploadedImages = updated.filter((item): item is ImageFile => !item.id.startsWith('default-'));
      onImagesChange?.(uploadedImages);
      return updated;
    });
  }, [onImagesChange]);
