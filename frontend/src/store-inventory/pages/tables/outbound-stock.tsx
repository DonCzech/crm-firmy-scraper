...
function mapProductsToOutboundRows(products: BackendProduct[]): OutboundStockData[] {
  return (products || []).map((product, index) => ({
    id: product.id,
    dateOrder: `SO-${(product.sku || `PRD-${index + 1}`).slice(0, 12).toUpperCase()}`,
    productInfo: {
    ...
    qty: '1',
    status: {
      label: product.status === 'published' ? 'Shipped' : 'Pending',
      variant: product.status === 'published' ? 'success' : 'warning',
    },
    expDelivery: formatStockDate(product.updatedAt),
    warehouse: (product.category || 'Main').slice(0, 10).toUpperCase(),
    carrier: 'DPD',
    notify: true,
  }));
}
...
export function OutboundStockTable({
  ...
  const [apiData, setApiData] = useState<OutboundStockData[]>([]);
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (!active) return;
        setApiData(mapProductsToOutboundRows(response.data || []));
      } catch {
        if (!active) return;
        setApiData([]);
      }
          ...
          <CardHeader className="py-3.5">
            ...
            <CardToolbar>
              <Button
                variant="outline"
                onClick={() => {
                  setCreateModalData(data[0] ?? null);
