...
const CATEGORY_META: Record<string, { name: string; color: string }> = {
  '1': { name: 'General', color: 'bg-slate-100 text-slate-700' },
  '2': { name: 'Services', color: 'bg-blue-100 text-blue-700' },
  '3': { name: 'Finance', color: 'bg-emerald-100 text-emerald-700' },
  '9': { name: 'Real Estate', color: 'bg-violet-100 text-violet-700' },
};

const CONNECTION_META: Record<string, { name: string; color: string }> = {
  '1': { name: 'Low', color: 'bg-red-500' },
  '2': { name: 'Medium', color: 'bg-amber-500' },
  '3': { name: 'High', color: 'bg-green-500' },
};

const ARR_META: Record<string, { name: string; color: string }> = {
  '1': { name: '< 100k', color: 'bg-slate-100 text-slate-700' },
  '2': { name: '100k - 1M', color: 'bg-blue-100 text-blue-700' },
  '3': { name: '1M+', color: 'bg-emerald-100 text-emerald-700' },
};

const EMPLOYEE_META: Record<string, { name: string; color: string }> = {
  '1': { name: '1-10', color: 'bg-slate-100 text-slate-700' },
  '2': { name: '11-50', color: 'bg-blue-100 text-blue-700' },
  '3': { name: '51-200', color: 'bg-emerald-100 text-emerald-700' },
};

export function CompanyExtendedDetailsCompany() {
  const { company: resolvedCompany } = useCurrentCompany();
  const [isGeneralOpen, setIsGeneralOpen] = useState(true);
  const [isGeneralAllOpen, setIsGeneralAllOpen] = useState(false);
  const { contactsById } = useCompanyContacts();
  const company = resolvedCompany || EMPTY_COMPANY;
          ...
          <div className="grid grid-cols-5 gap-2.5">
            ...
            <div className="col-span-3">
              <div className="flex items-center gap-1">
                {company.categoryIds?.map((categoryId) => {
                  const badge = CATEGORY_META[String(categoryId)];
            ...
            <div className="col-span-3">
              <div className="flex truncate overflow-hidden gap-1.5">
                {company.contactIds?.map((contactId) => {
                  ...
                  const displayName = contact?.name || `Kontakt ${String(contactId).slice(0, 6)}`;
                  return (
                    <div
                      key={contactId}
            ...
            <div className="col-span-3">
              {(() => {
                ...
                const item = CONNECTION_META[String(value)];
            ...
            <div className="col-span-3">
              {(() => {
                ...
                const badge = ARR_META[String(value)];
          ...
          <div className="grid grid-cols-5 gap-2.5">
            ...
            <div className="col-span-3">
              {(() => {
                ...
                const badge = EMPLOYEE_META[String(value)];
