...
export function EventItem({
  ...
  if (view === "month") {
    return (
      ...
      >
        {children || (
          <span className="truncate flex items-center gap-1 w-full min-w-0">
            {event.priority && (
              <span className={cn("w-1.5 h-1.5 rounded-full shrink-0", PRIORITY_DOT[event.priority])} />
            )}
            {event.recurringId && (
              <RepeatIcon className="w-2.5 h-2.5 shrink-0 opacity-70" />
            )}
      ...
      >
        {durationMinutes < 45 ? (
          <div className="truncate flex items-center gap-1">
            ...
            {event.recurringId && <RepeatIcon className="w-2.5 h-2.5 shrink-0 opacity-70" />}
            <span className="truncate">
              {event.title}{" "}
              {showTime && (
  ...
  return (
    ...
    >
      <div className="text-sm font-medium flex items-center gap-1.5">
        {event.priority && (
          <span className={cn("w-2 h-2 rounded-full shrink-0", PRIORITY_DOT[event.priority])} />
        )}
        {event.recurringId && <RepeatIcon className="w-3 h-3 shrink-0 opacity-70" />}
