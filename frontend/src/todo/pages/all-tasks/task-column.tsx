...
import { COLUMN_TITLES } from "@/todo/mock";
...
export function TaskColumn({ value, tasks, isOverlay, onEditTask, ...props }: TaskColumnProps) {
  return (
    <KanbanColumn value={value} {...props} className="rounded-md border bg-card p-2.5 shadow-xs">
      ...
      <KanbanColumnContent value={value} className="flex flex-col gap-2.5 p-0.5">
        {tasks.map((task) => (
          <TaskCard key={task.id} task={task} asHandle={!isOverlay} onEdit={onEditTask} />
        ))}
      </KanbanColumnContent>
    </KanbanColumn>
  );
