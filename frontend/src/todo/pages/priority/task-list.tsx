import { Flame, AlertCircle, ArrowDown, Calendar, Pencil } from "lucide-react";
import {
  Card,
  CardContent,
  CardHeader,
...
function getPriorityIcon(priority: string) {
  switch (priority) {
    case 'high': return <Flame className="size-4 text-red-500" />;
    case 'medium': return <AlertCircle className="size-4 text-yellow-500" />;
    default: return <ArrowDown className="size-4 text-blue-500" />;
  }
}
...
export function TaskItem({ task, onToggle, onEdit }: TaskItemProps) {
  return (
    ...
    >
      ...
      {onEdit && (
        <Button type="button" mode="icon" variant="ghost" size="sm" onClick={() => onEdit(task.id)}>
...
export function TaskList({ tasks, onToggleTask, onEditTask }: TaskListProps) {
