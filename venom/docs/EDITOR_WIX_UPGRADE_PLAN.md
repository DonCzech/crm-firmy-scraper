...
```
PHASE:        PRO upgrade DONE (Fáze 0-4, commity PRO-0.1 az PRO-3d). Editor: audit fixy, violet design + theme switcher (violet/silver/indigo komplet palety), Phosphor ikony, historie verzí, responsive editace, globální textové styly, Moje sekce + ⌘C/⌘V sekcí.
NEXT TASK:    Backlog: formulářový builder, Moduly (CRM) implementace, hero inspektor Layout tab (responsive pro hero), per-breakpoint overlay pozice, AI klíč do env.
...
2026-07-03 | PRO-3a2 | Přepínač témat rozšířen na KOMPLETNÍ palety (uživatelův požadavek): silver/indigo přepínají celý shell (bg/surfaces/bordery/texty/glass/toolbar/canvas), nové tokeny --vs-glass-bg/--vs-toolbar-bg/--vs-canvas-*-rgb/--vs-canvas-deep-*, utilita .vs-canvas-deep nahradila 4 hardcoded gradienty. Violet default pixel-identický. | 6
2026-07-03 | PRO-3d | Moje sekce: saved_sections + API, Bookmark tlačítko v section toolbaru (popover s názvem), kategorie "★ Moje sekce" ve WixAddOverlay (vložení se snapshotem settings, mazání). ⌘C/⌘V sekcí přes localStorage clipboard (funguje mezi stránkami, guard na text selection). StudioToast (event venom-studio:toast). Fix: section toolbar překrytý inspektorem — měřený posun doleva (canvas zoom stacking context). | 7
2026-07-03 | PRO-3a3 | Vzhled editoru: vlastní barva — 4. swatch s nativním color pickerem, editor-theme.ts buildCustomThemeCss() odvozuje kompletní paletu z jedné barvy (HSL, hue-shift gradienty, clamp sytosti), dynamický <style id=vs-custom-theme> + data-vs-theme=custom, persist obou klíčů v localStorage. | 4
