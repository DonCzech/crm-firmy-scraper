#!/usr/bin/env node
/**
 * Seed bakery-01 master tenant (fixed slug: bakery-01-v2).
 * - Removes previous bakery-01-v2 tenant
 * - POSTs onboarding with templateKey=bakery-01
 */
import pg from 'pg';
...
const SLUG = 'bakery-01-v2';
const EMAIL = 'demo@bakery-01.test';
...
async function main() {
  ...
  const res = await fetch(`${baseUrl}/api/onboarding`, {
    ...
    body: JSON.stringify({ email: EMAIL, templateKey: 'bakery-01', industry: 'bakery', slug: SLUG }),
  });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) { console.error(`✗ onboarding ${res.status}:`, body); process.exit(1); }
  console.log(`✓ bakery-01 tenant created`);
