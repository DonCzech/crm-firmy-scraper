/**
 * Mirror gerberra.cz — zahradnické práce, trávníky, výsadba
 * Run: node scripts/mirror-gerberra-assets.mjs (from venom/ dir)
 */
...
const SLUG   = 'gerberra';
const ORIGIN = 'https://gerberra.cz';
...
const LINK_MAP = {
  '/':                  `/demo/gerberra-demo`,
  '/sluzby/':           `/demo/gerberra-demo/sluzby`,
  '/sluzby':            `/demo/gerberra-demo/sluzby`,
  '/reference/':        `/demo/gerberra-demo/reference`,
  '/reference':         `/demo/gerberra-demo/reference`,
  '/kontakt/':          `/demo/gerberra-demo/kontakt`,
  '/kontakt':           `/demo/gerberra-demo/kontakt`,
  '/o-nas/':            `/demo/gerberra-demo`,
  '/o-nas':             `/demo/gerberra-demo`,
  '/galerie/':          `/demo/gerberra-demo/reference`,
  '/galerie':           `/demo/gerberra-demo/reference`,
  '/poptavka/':         `/demo/gerberra-demo/kontakt`,
  '/poptavka':          `/demo/gerberra-demo/kontakt`,
  '/realizace/':        `/demo/gerberra-demo/reference`,
  '/realizace':         `/demo/gerberra-demo/reference`,
};
...
function processHtml(html) {
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="https?://(?:www\\.)?gerberra\\.cz${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?gerberra\.cz\/[^"]*"/gi, `href="/demo/gerberra-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?gerberra\.cz"/gi, `href="/demo/gerberra-demo"`);
  html = html.replace(/https?:\/\/(?:www\.)?gerberra\.cz(\/[^"'\s>]+)/g, `/clones/${SLUG}$1`);
  ...
  // Brand scrub
  html = html.replace(/Gerberra(?!\s*Demo)/g, 'Gerberra Demo');
  html = html.replace(/gerberra\.cz/gi, 'demo.local');
  ...
  html = html.replace(/\/clones\/Gerberra Demo\//g, `/clones/${SLUG}/`);
  html = html.replace(/(\/clones\/${SLUG}\/[^\s"']*?)demo\.local/g, `$1gerberra.cz`);
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/gerberra\.cz/gi) || []).length;
