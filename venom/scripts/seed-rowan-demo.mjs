/**
 * FÁZE 2 — Seed DB pro rowan-legal-demo
 * WordPress Bedrock (advokátní kancelář), 4 stránky
 *
 * Spustit: node scripts/seed-rowan-demo.mjs
 */
...
const SLUG = 'rowan';
const TENANT_SLUG = 'rowan-legal-demo';
const ACCESS_TOKEN = 'rowan' + Math.random().toString(36).slice(2, 10);
...
const CSS_URLS = [
  `/clones/${SLUG}/app/themes/rowan/style.css`,
  `/clones/${SLUG}/app/themes/rowan/normalize.css`,
  `/clones/${SLUG}/app/themes/rowan/assets/css/default.css`,
].filter(u => fs.existsSync(`public${u}`));

const JS_URLS = []; // Skip — ajax calls
...
function patchHtml(rawHtml) {
  ...
  // Strip YouTube embeds
  body = body.replace(/<iframe[^>]*(?:youtube\.com|youtu\.be|youtube-nocookie\.com)[^>]*>[\s\S]*?<\/iframe>/gi,
  ...
  const jqueryInline = `<script src="/clones/${SLUG}/wp/wp-includes/js/jquery/jquery.min.js"></script>`;

  return KILL + '\n' + overrideCss + '\n' + jqueryInline + '\n' + inlineStyles + '\n' + body;
}

log('=== Seed rowan-legal-demo ===');
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo ROWAN LEGAL', 'advokátní kancelář Praha',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'rowan.legal', cms: 'WordPress Bedrock + custom Rowan theme' })]);
...
for (const p of PAGES) {
  ...
  const brand = (html.match(/rowan\.legal/gi) || []).length;
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo ROWAN LEGAL`, 'Ukázka šablony pro advokátní kancelář.']);
