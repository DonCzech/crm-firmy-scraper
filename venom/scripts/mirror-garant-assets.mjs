/**
 * Mirror autoservis-garant.cz — WordPress carservice theme
 * Run: node scripts/mirror-garant-assets.mjs
 */
...
const SLUG   = 'garant';
const ORIGIN = 'https://www.autoservis-garant.cz';
...
const PAGES = [
  ...
  { slug: 'reference', url: '/reference/ohlasy-zakazniku-zkusenosti/' },
];
...
for (const p of PAGES) {
  ...
  page.on('response', async (resp) => {
    ...
    if (isJs && /gtm|analytics|pixel|hotjar|cookiebot|gtag|cmplz/.test(base)) return;
    try {
      const dest = `${OUT}${rel}`;
  ...
  try { await page.click('[class*="cookie"] button, [id*="cookie"] button, button[class*="accept"], button[class*="cmplz-accept"]', { timeout: 2000 }); } catch {}
...
const LINK_MAP = {
  '/':                                       `/demo/autoservis-garant-demo`,
  '/o-nas':                                  `/demo/autoservis-garant-demo/o-nas`,
  '/o-nas/':                                 `/demo/autoservis-garant-demo/o-nas`,
  '/kontakt':                                `/demo/autoservis-garant-demo/kontakt`,
  '/kontakt/':                               `/demo/autoservis-garant-demo/kontakt`,
  '/reference':                              `/demo/autoservis-garant-demo/reference`,
  '/reference/':                             `/demo/autoservis-garant-demo/reference`,
  '/reference/ohlasy-zakazniku-zkusenosti/': `/demo/autoservis-garant-demo/reference`,
  '/reference/ukazky-realizaci/':            `/demo/autoservis-garant-demo/reference`,
  '/autodiagnostika':                        `/demo/autoservis-garant-demo`,
  '/autoelektrika':                          `/demo/autoservis-garant-demo`,
  '/klimatizace':                            `/demo/autoservis-garant-demo`,
  '/lakovani-aut':                           `/demo/autoservis-garant-demo`,
  '/garancni-prohlidka/':                    `/demo/autoservis-garant-demo`,
...
function processHtml(html) {
  ...
  html = html.replace(/(\/clones\/garant\/[^"'?#\s]+)\?[^"'>\s]*/g, '$1');

  html = html.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|cookiebot|cmplz|analytics)[^>]*>[\s\S]*?<\/script>/gi, '');
  html = html.replace(/<noscript><iframe[^>]*googletagmanager[^>]*><\/iframe><\/noscript>/gi, '');

  html = html.replace(/<div[^>]*(?:cmplz-cookiebanner|cookie-banner|cookie-consent|gdpr-banner)[^>]*>[\s\S]{0,5000}?<\/div>/gi, '');
  ...
  html = html.replace(/href="https?:\/\/(?:www\.)?autoservis-garant\.cz\/[^"]*"/gi, `href="/demo/autoservis-garant-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?autoservis-garant\.cz"/gi, `href="/demo/autoservis-garant-demo"`);

  // Brand scrub
  html = html.replace(/Autoservis\s+GARANT(?!\s*Demo)/g, 'Autoservis GARANT Demo');
  html = html.replace(/Autoservis\s+Garant(?!\s*Demo)/g, 'Autoservis Garant Demo');
  html = html.replace(/AUTOSERVIS\s+GARANT(?!\s*DEMO)/g, 'AUTOSERVIS GARANT DEMO');
  html = html.replace(/autoservis-garant\.cz/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@autoservis-garant\.cz/gi, 'info@demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/autoservis-garant\.cz/gi) || []).length;
