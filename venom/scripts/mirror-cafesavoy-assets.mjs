...
const ORIGIN = 'https://cafesavoy.ambi.cz';
...
const LINK_MAP = {
  '/':            `/demo/cafe-savoy-demo`,
  '/menu':        `/demo/cafe-savoy-demo/menu`,
  '/rezervace':   `#rezervace`,
};
...
function processHtml(html) {
  ...
  html = html.replace(/href="https?:\/\/(?:www\.|cafe-savoy\.|cafesavoy\.)?ambi\.cz\/[^"]*"/gi, `href="/demo/cafe-savoy-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.|cafe-savoy\.|cafesavoy\.)?ambi\.cz"/gi, `href="/demo/cafe-savoy-demo"`);

  // Brand scrub
  // Brand scrub — Café Savoy
  html = html.replace(/Café Savoy(?!\s*Demo)/g, 'Café Savoy Demo');
  html = html.replace(/Cafe Savoy(?!\s*Demo)/g, 'Cafe Savoy Demo');
  html = html.replace(/CAFÉ SAVOY(?!\s*DEMO)/g, 'CAFÉ SAVOY DEMO');
  html = html.replace(/Ambiente(?!\s*Demo)/g, 'Ambiente Demo');
  html = html.replace(/ambi\.cz/gi, 'demo.local');
  // Don't scrub /clones/cafesavoy/ paths - only scrub cafesavoy as standalone domain reference
  html = html.replace(/cafesavoy\.ambi\.cz/gi, 'cafesavoy.demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9.]+@ambi\.cz/gi, 'info@demo.local');
  html = html.replace(/[a-z0-9.]+@cafesavoy[^"'\s]*/gi, 'info@demo.local');
  html = html.replace(/savoy@demo\.local/gi, 'info@demo.local');
