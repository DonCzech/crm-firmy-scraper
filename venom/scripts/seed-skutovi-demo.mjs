/**
 * FÁZE 2 — Seed DB pro reality-skutovi-demo
 * Webflow CMS (realitní kancelář), 4 stránky
 *
 * Spustit: node scripts/seed-skutovi-demo.mjs
 */
...
const SLUG = 'skutovi';
const TENANT_SLUG = 'reality-skutovi-demo';
const ACCESS_TOKEN = 'skutovi' + Math.random().toString(36).slice(2, 10);
...
const CSS_URLS = [
  `/clones/${SLUG}/cdn/6721087ffa7841e21e09f4f7/css/skutovi.webflow.shared.59551ac3b.min.css`,
].filter(u => fs.existsSync(`public${u}`));

const JS_URLS = [
  `/clones/${SLUG}/cdn/6721087ffa7841e21e09f4f7/js/webflow.schunk.36b8fb49256177c8.js`,
  `/clones/${SLUG}/cdn/6721087ffa7841e21e09f4f7/js/webflow.bab92843.466fe5d817567381.js`,
].filter(u => fs.existsSync(`public${u}`));
...
const PAGES = [
  ...
  { slug: 'nabidka-prodej',   title: 'Nabídka prodej',  file: 'nabidka-prodej.html',   isHome: false },
  { slug: 'nabidka-pronajem', title: 'Nabídka pronájem', file: 'nabidka-pronajem.html', isHome: false },
...
function patchHtml(rawHtml) {
  ...
  // Strip inline tracking scripts
  body = body.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|smartsupp|firmy\.cz|connect\.facebook)[^>]*>[\s\S]*?<\/script>/gi, '');
  ...
  // JS reveal: remove inline opacity:0 from Webflow scroll-reveal elements
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Reality Škutovi', 'rodinná realitní kancelář',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'realityskutovi.cz', cms: 'Webflow' })]);
...
for (const p of PAGES) {
  ...
  const brand = (html.match(/realityskutovi\.cz/gi) || []).length;
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo Reality Škutovi`, 'Ukázka šablony pro rodinnou realitní kancelář.']);
