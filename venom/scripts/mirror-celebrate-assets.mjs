/**
 * Mirror celebratesalon.cz — Wix Thunderbolt SPA
 */
...
const ORIGIN = 'https://www.celebratesalon.cz';
const OUT = `public/clones/${SLUG}`;
const PAGES = [
  { slug: 'home',     url: '/' },
  { slug: 'manikura', url: '/manikura' },
  { slug: 'onas',     url: '/onas' },
];
...
for (const p of PAGES) {
  ...
  page.on('response', async (resp) => {
    ...
    const isWixAsset = url.includes('wixstatic.com') || url.includes('frog.wix.com');
...
function processHtml(html) {
  ...
  html = html.replaceAll('https://www.celebratesalon.cz', '');
  return html;
}
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/celebratesalon\.cz/gi) || []).length;
  const wixLeft = (processed.match(/wixstatic\.com/gi) || []).length;
  console.log(`${p.slug}: ${raw.length}→${processed.length} | brand=${brandLeft} wix=${wixLeft}`);
}
console.log('\nMirror done ✅');
