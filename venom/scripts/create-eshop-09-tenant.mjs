/**
 * Create tenant eshop-09-v2 for the eshop-09 (Mobil Pohotovost DNA) template.
 * Idempotent: reuses existing tenant, re-seeds pages/sections from manifest.
 * Usage: node scripts/create-eshop-09-tenant.mjs
 */
import pg from "pg";
...
const TEMPLATE_KEY = "eshop-09";
const TENANT_SLUG = "eshop-09-v2";
const SHOP_NAME = "Mobil Expres";
...
try {
  ...
  for (const page of manifest.pages) {
    ...
    // Podstránky = legacy: jejich varianty (eshop-09-contact, -text, …) nejsou
  ...
  console.log(`\n✅ ${TENANT_SLUG} ready → http://localhost:3016/demo/${TENANT_SLUG}`);
} catch (err) {
