/**
 * Seed elektrobohacek-demo — WordPress + Elementor + Autoptimize
 * Run: node -e "import('/Users/apple/DEV/CRM/venom/scripts/seed-elektrobohacek-demo.mjs')"
 */
...
const TENANT_SLUG = 'elektrobohacek-demo';
const ACCESS_TOKEN = 'elektro' + Math.random().toString(36).slice(2, 10);
...
const CSS_BASE = path.join(ROOT, `public/clones/${SLUG}/wp-content/cache/autoptimize/css`);
const CSS_URLS = fs.existsSync(CSS_BASE)
  ? fs.readdirSync(CSS_BASE).filter(f => f.endsWith('.css')).map(f => `/clones/${SLUG}/wp-content/cache/autoptimize/css/${f}`)
  : [];

const JS_BASE = path.join(ROOT, `public/clones/${SLUG}/wp-content/cache/autoptimize/js`);
const JS_URLS = fs.existsSync(JS_BASE)
  ? fs.readdirSync(JS_BASE).filter(f => f.endsWith('.js')).sort().map(f => `/clones/${SLUG}/wp-content/cache/autoptimize/js/${f}`)
  : [];
...
const PAGES = [
  ...
  { slug: 'elektroinstalace', title: 'Elektroinstalace',  file: 'elektroinstalace.html',  isHome: false },
  { slug: 'hromosvody',       title: 'Hromosvody',        file: 'hromosvody.html',        isHome: false },
];
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Elektro Boháček', 'elektroinstalace Praha',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'elektro-bohacek.cz', cms: 'WordPress + Elementor + Autoptimize' })]);
...
for (const p of PAGES) {
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo Elektro Boháček`, 'Ukázka šablony pro elektroinstalace Praha.']);
