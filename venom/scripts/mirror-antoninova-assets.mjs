/**
 * Mirror antoninovopekarstvi.cz — Pekárna
...
console.log('Navigating antoninovopekarstvi.cz...');
await page.goto(ORIGIN, { waitUntil: 'networkidle', timeout: 35000 });

// Dismiss cookie consent
try {
  await page.click('[data-testid="accept"], #cookie-accept, .cookie-accept, button[id*="accept"], .btn-accept, button:has-text("Souhlasím"), button:has-text("Přijmout"), button:has-text("Souhlasit"), .accept-cookies, #acceptCookies', { timeout: 4000 });
...
try {
  ...
  await page.click('.modal-close, .close, [data-dismiss="modal"], .popup-close, .fancybox-close', { timeout: 1500 });
} catch {}
...
const isWordPress = rawLower.includes('wp-content') || rawLower.includes('wp-includes');
const isJoomla = rawLower.includes('joomla') || rawLower.includes('/templates/');
console.log(`CMS detection: WordPress=${isWordPress} Joomla=${isJoomla}`);

// --- Build home.html ---
function buildHtml(raw) {
  // Split AFTER </head> to avoid matching <body> inside JS comments
  const headEnd = raw.indexOf('</head>');
  const afterHead = headEnd > -1 ? raw.slice(headEnd + 7) : raw;
  const bodyM = afterHead.match(/<body[^>]*>([\s\S]*)<\/body>/i);
  let body = bodyM ? bodyM[1] : afterHead;

  // Strip tracking
  body = body
    .replace(/<script[^>]*(?:googletagmanager|gtag|analytics|tawk|hotjar|cookiebot|facebook|doubleclick)[^>]*>[\s\S]*?<\/script>/gi, '')
  ...
  body = body
    ...
    .replace(/Antonínovo\s+pekárenství/gi, 'Demo pekárenství')
...
<head>
  ...
  <title>Demo Pekárenství — Ukázka šablony</title>
  <meta name="description" content="Ukázka prémiové webové šablony pro pekárnu.">
...
const brandRefs = (finalHtml.match(/antoninovopekarstvi\.cz/gi) || []).length;
