/**
 * Create tenant eshop-20-v2 for the eshop-20 (Dedoles DNA — Vykuk veselá móda) template.
 * Idempotent: reuses existing tenant, re-seeds pages/sections from manifest.
 * Usage: node scripts/create-eshop-20-tenant.mjs
 */
import pg from "pg";
...
const TEMPLATE_KEY = "eshop-20";
const TENANT_SLUG = "eshop-20-v2";
const SHOP_NAME = "Vykuk";
...
try {
  ...
  // Dedoles-like doprava (footer chipy Výdejní box/Kurýr/Pošta, zdarma od 999 Kč)
  // + platby vč. expresních Apple/Google Pay a dobírky (pokladna.pdf)
  const SHIPPING_METHODS = [
    { key: "vydejni-box", label: "Výdejní box", description: "Zásilkovna / AlzaBox — vyzvednutí do 1–2 dnů", price_cents: 6900, free_above_cents: 99900, enabled: true },
    { key: "kuryr", label: "Kurýr na adresu", description: "Doručení domů do 1–2 pracovních dnů", price_cents: 9900, free_above_cents: 99900, enabled: true },
    { key: "posta", label: "Česká pošta — Balík Do ruky", description: "Doručení do 2–3 pracovních dnů", price_cents: 8900, free_above_cents: 99900, enabled: true },
  ];
  const PAYMENT_METHODS = [
    { key: "applepay", label: "Apple Pay", description: "Expresní platba jedním dotykem", fee_cents: 0, enabled: true },
    { key: "googlepay", label: "Google Pay", description: "Expresní platba jedním dotykem", fee_cents: 0, enabled: true },
    { key: "karta", label: "Kreditní/debetní karta", description: "Visa, Mastercard — zabezpečená platba", fee_cents: 0, enabled: true },
    { key: "cod", label: "Platba na dobírku", description: "Zaplatíte při převzetí", fee_cents: 3900, enabled: true },
    { key: "bank_transfer", label: "Bankovní převod", description: "Zboží odešleme po připsání platby", fee_cents: 0, enabled: true },
  ];
  await client.query(
    `UPDATE shops SET settings = COALESCE(settings, '{}'::jsonb)
       || jsonb_build_object('shipping_methods', $2::jsonb, 'payment_methods', $3::jsonb),
       updated_at = now()
     WHERE tenant_id = $1`,
    [tenantId, JSON.stringify(SHIPPING_METHODS), JSON.stringify(PAYMENT_METHODS)]
  );
  ...
  // Chytré vyhledávání (našeptávač) + top obsah v prázdném stavu
  await client.query(
    `INSERT INTO commerce_addon_activations (tenant_id, addon_slug, enabled)
     VALUES ($1, 'chytre-vyhledavani', true)
     ON CONFLICT (tenant_id, addon_slug) DO UPDATE SET enabled = true`,
    [tenantId]
  );
  console.log("✓ Addon chytre-vyhledavani aktivován");
  ...
  for (const page of manifest.pages) {
    ...
    // Homepage = v2 (default_sections resolver, sparse overrides pro studio).
    // Podstránky = legacy: jejich varianty (eshop-20-contact, -text, …) nejsou
    // v home-only default_sections, takže v2 resolver by je matchoval špatně
    // (fallback na první about = fav-categories). Legacy čte settings.content
    // přímo. Stejný vzor jako funkční eshop-06 podstránky.
