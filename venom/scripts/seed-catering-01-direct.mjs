#!/usr/bin/env node
/**
 * Direct DB seed for catering-01 — bypasses the /api/onboarding route
 * (production server has compiled enum without elektro-01).
...
const SLUG         = 'catering-01-v2';
const EMAIL        = 'demo@catering-01.test';
const TEMPLATE_KEY = 'catering-01';
const INDUSTRY     = 'catering';
  ...
  try {
    ...
    const pageRes = await client.query(
      `INSERT INTO pages (tenant_id, slug, title, is_homepage, status)
       ...
       RETURNING id`,
      [tenantId, tpl.demoContent.siteName ?? 'Demo Catering Praha']
    );
