/**
 * Mirror elektro-bohacek.cz — elektroinstalace Praha
 * Run: node -e "import('/Users/apple/DEV/CRM/venom/scripts/mirror-elektrobohacek-assets.mjs')"
 */
...
const SLUG   = 'elektrobohacek';
const ORIGIN = 'https://elektro-bohacek.cz';
const OUT    = path.join(__dirname, '..', 'public', 'clones', SLUG);

const PAGES = [
  { slug: 'home',               url: '/' },
  { slug: 'elektroinstalace',   url: '/elektroinstalace/' },
  { slug: 'hromosvody',         url: '/hromosvody/' },
];
...
for (const p of PAGES) {
  ...
  try {
    ...
    console.log('  timeout/redirect:', e.message.substring(0,60));
  }
...
const LINK_MAP = {
  '/':                      `/demo/elektrobohacek-demo`,
  '/#sluzby':               `/demo/elektrobohacek-demo`,
  '/#reference':            `/demo/elektrobohacek-demo`,
  '/elektroinstalace/':     `/demo/elektrobohacek-demo/elektroinstalace`,
  '/elektroinstalace':      `/demo/elektrobohacek-demo/elektroinstalace`,
  '/hromosvody/':           `/demo/elektrobohacek-demo/hromosvody`,
  '/hromosvody':            `/demo/elektrobohacek-demo/hromosvody`,
  '/kontakt/':              `/demo/elektrobohacek-demo`,
  '/kontakt':               `/demo/elektrobohacek-demo`,
};
...
function processHtml(html) {
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="https?://(?:www\\.)?elektro-bohacek\\.cz${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?elektro-bohacek\.cz\/[^"]*"/gi, `href="/demo/elektrobohacek-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?elektro-bohacek\.cz"/gi, `href="/demo/elektrobohacek-demo"`);
  ...
  // Brand scrub — case-sensitive only to avoid corrupting /clones/elektrobohacek/ paths
  html = html.replace(/Elektro[- ]?Boháček(?!\s*Demo)/g, 'Elektro Boháček Demo');
  html = html.replace(/Elektro[- ]?Bohacek(?!\s*Demo)/g, 'Elektro Bohacek Demo');
  html = html.replace(/elektro-bohacek\.cz/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@elektro-bohacek\.cz/gi, 'info@demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/elektro-bohacek\.cz/gi) || []).length;
