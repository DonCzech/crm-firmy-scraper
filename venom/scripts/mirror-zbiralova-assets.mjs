...
const BASE_URL = 'https://zbiralova.cz';
const CLONE_DIR = '/Users/apple/DEV/CRM/venom/public/clones/zbiralova';
const PAGES_DIR = path.join(CLONE_DIR, 'pages');
fs.mkdirSync(PAGES_DIR, { recursive: true });

const PAGES = [
  { slug: 'home',     url: '/' },
  { slug: 'svatby',   url: '/svatby' },
  { slug: 'rodiny',   url: '/rodiny' },
  { slug: 'o-mne',    url: '/o-mne' },
  { slug: 'cenik',    url: '/sluzby-ceny' },
];
...
context.on('response', async (response) => {
  ...
  if (!url.includes('zbiralova.cz')) return;
  ...
  assetMap[url.split('?')[0]] = `/clones/zbiralova${urlPath}`;
