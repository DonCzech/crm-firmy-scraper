/**
 * Mirror klempirzprahy.cz — klempíř, pokrývač, střechy Praha
 * Run: node scripts/mirror-klempirzprahy-assets.mjs (from venom/ dir)
 */
...
const SLUG   = 'klempirzprahy';
const ORIGIN = 'https://klempirzprahy.cz';
const OUT    = path.join(__dirname, '..', 'public', 'clones', SLUG);

const PAGES = [
  { slug: 'home',      url: '/' },
  { slug: 'sluzby',    url: '/sluzby/klempir-praha' },
];
...
const LINK_MAP = {
  '/':                  `/demo/klempirzprahy-demo`,
  '/sluzby/':           `/demo/klempirzprahy-demo/sluzby`,
  '/sluzby':            `/demo/klempirzprahy-demo/sluzby`,
  '/reference/':        `/demo/klempirzprahy-demo/reference`,
  '/reference':         `/demo/klempirzprahy-demo/reference`,
  '/kontakt/':          `/demo/klempirzprahy-demo/kontakt`,
  '/kontakt':           `/demo/klempirzprahy-demo/kontakt`,
  '/o-nas/':            `/demo/klempirzprahy-demo`,
  '/o-nas':             `/demo/klempirzprahy-demo`,
  '/cenik/':            `/demo/klempirzprahy-demo/kontakt`,
  '/cenik':             `/demo/klempirzprahy-demo/kontakt`,
  '/galerie/':          `/demo/klempirzprahy-demo/reference`,
  '/galerie':           `/demo/klempirzprahy-demo/reference`,
  '/poptavka/':         `/demo/klempirzprahy-demo/kontakt`,
  '/poptavka':          `/demo/klempirzprahy-demo/kontakt`,
};
...
function processHtml(html) {
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="https?://(?:www\\.)?klempirzprahy\\.cz${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?klempirzprahy\.cz\/[^"]*"/gi, `href="/demo/klempirzprahy-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?klempirzprahy\.cz"/gi, `href="/demo/klempirzprahy-demo"`);
  html = html.replace(/https?:\/\/(?:www\.)?klempirzprahy\.cz(\/[^"'\s>]+)/g, `/clones/${SLUG}$1`);
  ...
  // Brand scrub
  html = html.replace(/Klempíř Praha(?!\s*Demo)/gi, 'Klempíř Praha Demo');
  html = html.replace(/klempirzprahy\.cz/gi, 'demo.local');
  ...
  html = html.replace(/\/clones\/Klempíř Praha Demo\//gi, `/clones/${SLUG}/`);
  html = html.replace(/(\/clones\/${SLUG}\/[^\s"']*?)demo\.local/g, `$1klempirzprahy.cz`);
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/klempirzprahy\.cz/gi) || []).length;
