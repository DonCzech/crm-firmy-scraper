#!/usr/bin/env node
/**
 * Seed autoskola-01 master tenant (slug: autoskola-01-v2).
 * Template: autoskola-01 — Demo Autoškola DRIVE (oranžový #f16823 akcent, Roboto)
 */
import pg from 'pg';
...
const SLUG = 'autoskola-01-v2';
const EMAIL = 'demo@autoskola-01.test';
const TEMPLATE_KEY = 'autoskola-01';
const INDUSTRY = 'autoskola';
...
async function main() {
  ...
  const res = await fetch(`${baseUrl}/api/onboarding`, {
    ...
    body: JSON.stringify({
      businessName: 'Demo Autoškola DRIVE',
      email: EMAIL,
