#!/usr/bin/env node
/**
 * Direct DB seed for video-01 — bypasses /api/onboarding
 * video-01 — Demo Kameraman (bílý #fff, zlatý #C49A6C, tmavý #2E2A28, Playfair+Inter)
 */
import pg from 'pg';
...
const SLUG         = 'video-01-demo';
const EMAIL        = 'demo@video-01.test';
const TEMPLATE_KEY = 'video-01';
const INDUSTRY     = 'video';
...
function themeToDesignTokens(theme) {
  return {
    ...
    spacing:         ({ compact:'compact', normal:'normal', spacious:'relaxed', editorial:'relaxed', airy:'relaxed' })[theme.spacing.personality] ?? 'normal',
  };
}
...
function buildTemplate(manifest, theme, content) {
  ...
  return { key: manifest.key, name: manifest.name, industry: manifest.industry,
           version: manifest.version, designTokens, defaultSections, pages: subPages, demoContent: content };
}
...
async function main() {
  ...
  try {
    ...
    await client.query(
      ...
      [templateId, tpl.version, JSON.stringify(tpl.defaultSections),
       JSON.stringify(tpl.designTokens), JSON.stringify(tpl.demoContent)]
    );
