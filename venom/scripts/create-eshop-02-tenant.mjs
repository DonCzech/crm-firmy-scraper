/**
 * Create tenant eshop-02-v2 for the eshop-02 (Shoptet Classic) template.
 * - tenant (tenant_kind commerce) + shop + core commerce modules
 * - pages + sections from src/templates/eshop-02/template.json (legacy settings)
 * Idempotent: reuses existing tenant, re-seeds pages/sections from manifest.
 * Usage: node scripts/create-eshop-02-tenant.mjs
 */
import pg from "pg";
...
const TEMPLATE_KEY = "eshop-02";
const TENANT_SLUG = "eshop-02-v2";
const SHOP_NAME = "Modrý Košík";
...
try {
  ...
  if (existing.rows.length) {
    ...
    console.log(`✓ Tenant created: ${TENANT_SLUG} (id=${tenantId}), access_token=${accessToken}`);
  }
  ...
  for (const page of manifest.pages) {
    ...
    for (let i = 0; i < page.sections.length; i++) {
      ...
      await client.query(
        ...
        [tenantId, pageRes.rows[0].id, s.type, s.variant, i, JSON.stringify(settings)]
      );
    }
