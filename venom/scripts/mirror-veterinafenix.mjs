/**
 * FÁZE 1 — mirror veterinafenix.cz (WordPress)
 */
...
const ORIGIN = 'https://www.veterinafenix.cz';
...
console.log('Navigating veterinafenix.cz...');
await page.goto(ORIGIN, { waitUntil: 'networkidle', timeout: 30000 });
await page.waitForTimeout(3000);

// Dismiss cookie
try {
  await page.click('button:has-text("Přijmout"), button:has-text("Souhlasím"), button:has-text("OK"), .cookie-accept, #cookie-accept, .cmplz-accept', { timeout: 3000 });
...
await page.screenshot({ path: '/tmp/veterinafenix-orig-full.jpg', fullPage: true });
...
console.log(`CMS: WordPress=${rawLower.includes('wp-content')} Complianz=${rawLower.includes('complianz')}`);
