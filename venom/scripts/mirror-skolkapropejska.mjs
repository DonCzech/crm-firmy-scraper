/**
 * FÁZE 1 — mirror skolkapropejska.cz (WordPress, hotel pro psy)
 */
...
const ORIGIN = 'https://www.skolkapropejska.cz';
...
console.log('Navigating skolkapropejska.cz...');
await page.goto(ORIGIN, { waitUntil: 'networkidle', timeout: 30000 });
await page.waitForTimeout(3000);

// Dismiss cookie / popup
try {
  await page.click('button:has-text("Přijmout"), button:has-text("Souhlasím"), .cookie-accept, #cookie-accept, .cmplz-accept, [aria-label="Close"]', { timeout: 3000 });
  console.log('Overlay dismissed');
} catch { console.log('No overlay'); }

// Scroll
await page.evaluate(async () => {
  for (let i = 0; i < 25; i++) { window.scrollBy(0, window.innerHeight); await new Promise(r => setTimeout(r, 200)); }
  window.scrollTo(0, 0);
});
...
await page.screenshot({ path: '/tmp/skolkapropejska-orig-full.jpg', fullPage: true });
...
console.log(`Complianz=${lower.includes('complianz')} Elementor=${lower.includes('elementor')} Wix=${lower.includes('data-mesh-id')}`);
