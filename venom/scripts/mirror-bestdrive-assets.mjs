/**
 * Mirror bestdrive.cz — Adobe AEM (autoservis řetězec, Continental)
 * Run: node scripts/mirror-bestdrive-assets.mjs
 */
...
const ORIGIN = 'https://www.bestdrive.cz';
const OUT    = `public/clones/${SLUG}`;

const PAGES = [
  { slug: 'home',       url: '/' },
  { slug: 'prodejny',   url: '/prodejny.html' },
  { slug: 'akce',       url: '/akce.html' },
  { slug: 'pneuservis', url: '/services/pneuservis.html' },
];
...
['etc.clientlibs','content','assets','static'].forEach(d => walk(`${OUT}/${d}`, (fp) => { if (fp.endsWith('.css')) fs.writeFileSync(fp, fixCss(fs.readFileSync(fp, 'utf8'))); }));

const LINK_MAP = {
  '/':                          `/demo/best-drive-demo`,
  '/prodejny.html':             `/demo/best-drive-demo/prodejny`,
  '/akce.html':                 `/demo/best-drive-demo/akce`,
  '/services/pneuservis.html':  `/demo/best-drive-demo/pneuservis`,
  '/klub.html':                 `/demo/best-drive-demo`,
};

function processHtml(html) {
  for (const [orig, local] of assetMap) html = html.replaceAll(orig, local);
  html = html.replace(/(\/clones\/bestdrive\/[^"'?#\s]+)\?[^"'>\s]*/g, '$1');

  html = html.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|cookiebot|usercentrics|analytics)[^>]*>[\s\S]*?<\/script>/gi, '');
  ...
  // Strip individual branch + offer detail pages → home
  html = html.replace(/href="\/prodejny\/[^"]*"/gi, `href="/demo/best-drive-demo/prodejny"`);
  html = html.replace(/href="\/akce\/[^"]*"/gi, `href="/demo/best-drive-demo/akce"`);
  html = html.replace(/href="\/branch-services\/[^"]*"/gi, `href="/demo/best-drive-demo"`);
  html = html.replace(/href="\/services\/[^"]*"(?!\/clones\/)/gi, `href="/demo/best-drive-demo"`);
  html = html.replace(/href="\/menu\/[^"]*"/gi, `href="/demo/best-drive-demo"`);
  html = html.replace(/href="\/fleet\/[^"]*"/gi, `href="/demo/best-drive-demo"`);

  for (const [from, to] of Object.entries(LINK_MAP)) {
    html = html.replace(new RegExp(`href="${from.replace(/\//g,'\\/').replace(/\./g,'\\.')}"`, 'gi'), `href="${to}"`);
    html = html.replace(new RegExp(`href="${ORIGIN.replace(/\./g,'\\.')}${from.replace(/\//g,'\\/').replace(/\./g,'\\.')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  // Remaining .html links → demo home
  html = html.replace(/href="\/[a-z0-9/_-]+\.html"/gi, `href="/demo/best-drive-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?bestdrive\.cz\/[^"]*"/gi, `href="/demo/best-drive-demo"`);

  // Brand scrub
  html = html.replace(/BestDrive(?!\s*Demo)/g, 'BestDrive Demo');
  html = html.replace(/BESTDRIVE(?!\s*DEMO)/g, 'BESTDRIVE DEMO');
  html = html.replace(/bestdrive\.cz/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@bestdrive\.cz/gi, 'info@demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/bestdrive\.cz/gi) || []).length;
