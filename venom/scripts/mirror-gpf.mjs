/**
 * FÁZE 1 — mirror gpf.cz (Nuxt SSR, hypoteční poradce — Gepard Finance)
 */
...
console.log('Navigating gpf.cz...');
await page.goto(ORIGIN, { waitUntil: 'networkidle', timeout: 45000 });
await page.waitForTimeout(4000);

try {
  await page.click('button:has-text("Přijmout"), button:has-text("Souhlasím"), .cookie-accept, #cookie-accept, [aria-label="Close"], button:has-text("OK")', { timeout: 3000 });
...
await page.screenshot({ path: '/tmp/gpf-orig-full.jpg', fullPage: true });
...
console.log(`Nuxt=${lower.includes('__nuxt')} Complianz=${lower.includes('complianz')} Elementor=${lower.includes('elementor')}`);
