#!/usr/bin/env node
/**
 * Direct DB seed for florist-01 — bypasses the /api/onboarding route
 * (production server has compiled enum without florist-01).
...
const SLUG         = 'florist-01-v2';
const EMAIL        = 'demo@florist-01.test';
const TEMPLATE_KEY = 'florist-01';
  ...
  try {
    ...
    const pageRes = await client.query(
      `INSERT INTO pages (tenant_id, slug, title, is_homepage, status)
       ...
       RETURNING id`,
      [tenantId, tpl.demoContent.siteName ?? 'Demo Květinářství']
    );
