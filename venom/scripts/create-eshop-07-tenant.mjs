/**
 * Create tenant eshop-07-v2 for the eshop-07 (Kosmetika-zdravi.cz DNA) template.
 * Idempotent: reuses existing tenant, re-seeds pages/sections from manifest.
 * Usage: node scripts/create-eshop-07-tenant.mjs
 */
import pg from "pg";
...
const TEMPLATE_KEY = "eshop-07";
const TENANT_SLUG = "eshop-07-v2";
const SHOP_NAME = "Néroli parfumerie";
...
try {
  ...
  for (const slug of ["chytre-vyhledavani", "hodnoceni-produktu", "stitky-v-obrazku", "oblibene-produkty"]) {
    await client.query(
      `INSERT INTO commerce_addon_activations (tenant_id, addon_slug, enabled) VALUES ($1, $2, true) ON CONFLICT DO NOTHING`,
      [tenantId, slug]
    );
  }
  console.log("✓ Addony: chytre-vyhledavani, hodnoceni-produktu, stitky-v-obrazku, oblibene-produkty");
  ...
  for (const page of manifest.pages) {
    ...
    for (let i = 0; i < page.sections.length; i++) {
      ...
      // template_versions.default_sections pokrývají JEN homepage — v2 resolver
      // by podstránkám podle (type, order) přiřadil špatný obsah. Podstránky
      // proto dostávají plný obsah do content_overrides (vrství se nad default).
      const overrides = page.isHomepage ? {} : sectionContent;
      await client.query(
        `INSERT INTO sections (tenant_id, page_id, section_type, section_variant, order_index, is_visible, settings, content_source, content_overrides)
         VALUES ($1, $2, $3, $4, $5, true, $6, 'v2', $7)`,
        [tenantId, pageRes.rows[0].id, s.type, s.variant, i, JSON.stringify(settings), JSON.stringify(overrides)]
      );
    }
