/**
 * FÁZE 2 — Seed DB pro engel-volkers-demo
 * Next.js SSR international real estate brand, 4 stránky
 *
 * Spustit: node scripts/seed-engelvolkers-demo.mjs
 */
...
const TENANT_SLUG = 'engel-volkers-demo';
const ACCESS_TOKEN = 'ev' + Math.random().toString(36).slice(2, 10);
...
const CSS_URLS = [];
const cssDir = `public/clones/${SLUG}/cdn/website.engelvoelkers.com/_next/static/css`;
if (fs.existsSync(cssDir)) {
  for (const f of fs.readdirSync(cssDir)) {
    if (f.endsWith('.css')) CSS_URLS.push(`/clones/${SLUG}/cdn/website.engelvoelkers.com/_next/static/css/${f}`);
  }
}

const JS_URLS = []; // Skip Next.js hydration JS

log(`CSS: ${CSS_URLS.length} | JS: ${JS_URLS.length}`);

// Depth-tracking div block strip
function stripDivBlock(html, startMatch) {
  ...
  while ((m = re.exec(html))) {
    if (m[0][1] === '/') {
      depth--;
      if (depth === 0) {
        return html.substring(0, startIdx) + html.substring(m.index + m[0].length);
      }
    } else depth++;
  }
  return html;
...
const PAGES = [
  ...
  { slug: 'prehled-sluzeb', title: 'Přehled služeb', file: 'prehled-sluzeb.html', isHome: false },
];
...
function patchHtml(rawHtml) {
  ...
  // Strip ALL <script> tags — Next.js hydration won't work
  body = body.replace(/<script[\s\S]*?<\/script>/gi, '');
  body = body.replace(/<noscript[^>]*>[\s\S]*?<\/noscript>/gi, '');

  // Strip Usercentrics + Cookiebot + Didomi via depth tracking
  body = stripDivBlock(body, /<div[^>]*id="(?:usercentrics-root|CybotCookiebotDialog|didomi-host|didomi-popup)"/i);
  body = stripDivBlock(body, /<div[^>]*class="[^"]*(?:usercentrics|cookiebot|didomi-popup|didomi-notice)[^"]*"/i);
  // Didomi can have nested popup view too
  body = stripDivBlock(body, /<div[^>]*class="didomi-popup-view"/i);
  // Strip Didomi backdrop
  body = body.replace(/<div[^>]*class="[^"]*didomi-popup-backdrop[^"]*"[^>]*>[\s\S]{0,500}?<\/div>/gi, '');

  // Strip privacy-center / cookie banner remnants
  body = body.replace(/<div[^>]*data-testid="[^"]*consent[^"]*"[^>]*>[\s\S]{0,8000}?<\/div>/gi, '');

  // CSS override
  const overrideCss = `<style>
    [id*="usercentrics"],[id*="Cookiebot"],[id*="Cybot"],[id*="didomi"],
    [class*="usercentrics"],[class*="Cookiebot"],[class*="Cybot"],
    [class*="didomi-popup"],[class*="didomi-notice"],[class*="didomi-host"],
    [data-testid*="consent"],[data-testid*="cookie"]{display:none!important;visibility:hidden!important}
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Engel & Völkers', 'realitní kancelář — mezinárodní luxus',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'engelvoelkers.com', cms: 'Next.js SSR + Storyblok CMS' })]);
...
for (const p of PAGES) {
  ...
  const brand = (html.match(/engelvoelkers\.com/gi) || []).length;
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo Engel & Völkers`, 'Ukázka šablony pro mezinárodní realitní kancelář.']);
