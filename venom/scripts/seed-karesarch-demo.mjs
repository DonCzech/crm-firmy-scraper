/**
 * FÁZE 2+3+7 — karesarch-demo (TYPO3 CMS, architektonický ateliér)
 */
import pg from 'pg';
...
const TENANT_SLUG = 'karesarch-demo';
...
// TYPO3 image paths: /fileadmin/, /typo3temp/, /uploads/
body = body.replace(/(?:https?:\/\/(?:www\.)?karesarch\.cz)?\/(?:fileadmin|typo3temp|uploads)\/[^\s"'<>?#]+\.(jpg|jpeg|png|webp|svg|gif|ico)/gi, (match) => {
...
// Fix internal links → #
body = body.replace(/href="https?:\/\/(?:www\.)?karesarch\.cz([^"]*)"/gi, 'href="#"');
...
body = body
  // Domain first
  .replace(/karesarch\.cz/gi, 'demo.local')
  // Brand variants
  .replace(/KAREŠ\s+ARCH/g, 'DEMO ARCH')
  .replace(/Kareš\s+Arch/g, 'Demo Arch')
  .replace(/KAREŠ/g, 'DEMO')
  .replace(/Kareš/g, 'Demo')
  .replace(/Martin\s+Kareš/g, 'Martin Demo')
  .replace(/Atelier\s+Kareš/gi, 'Demo Atelier')
  // Slug variants (karesarch → preserve path; for non-path use generic)
  // NOTE: 'kares' could be in many places, must guard against /clones/karesarch/ paths
  // ERIGO (web developer) → Demo Studio
  .replace(/ERIGO\.?/g, 'Demo Studio')
  .replace(/erigo\.cz/gi, 'demo.local')
  // Contact
  .replace(/\+420\s*603\s*904\s*732/g, '+420 608 288 777')
  .replace(/\+420\s*775\s*277\s*375/g, '+420 608 288 777')
  .replace(/atelier@(?:karesarch|demo)\.(?:cz|local)/gi, 'info@demo.local')
  // IČO
  .replace(/\b05051312\b/g, '12345678')
  .replace(/\b26943107\b/g, '12345678');

// Path corruption fix (slug 'karesarch' not directly replaced, but check)
// kares → demo-arch only on text, not paths
log(`Brand scrub done`);

// Style override
const styleOverride = `<style>
/* Venom karesarch demo */
...
const brand = (stripped.match(/[Kk]areš|KAREŠ|karesarch\.cz|Martin\s+Kareš/g) || []).length;
...
const ACCESS_TOKEN = 'kares' + Math.random().toString(36).slice(2, 8);
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Arch', 'Architektonický ateliér',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'karesarch.cz', cms: 'TYPO3' })]);
...
const pg2 = await pool.query(`
  ...
  VALUES ($1,'home','Domů',true,'Demo Arch — Architektonický ateliér','Ukázka šablony pro architektonický ateliér.') RETURNING id
`, [tid]);
