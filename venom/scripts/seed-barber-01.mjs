#!/usr/bin/env node
/**
 * Seed barber-01 demo tenant (fixed slug: barber-01-demo).
 * - Removes previous barber-01-demo tenant
 * - POSTs onboarding with templateKey=barber-01
 */
import pg from 'pg';
...
const SLUG = 'barber-01-demo';
const EMAIL = 'demo@barber-01.test';
...
async function main() {
  ...
  const res = await fetch(`${baseUrl}/api/onboarding`, {
    ...
    body: JSON.stringify({ email: EMAIL, templateKey: 'barber-01', industry: 'barber', slug: SLUG }),
  });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) { console.error(`✗ onboarding ${res.status}:`, body); process.exit(1); }
  console.log(`✓ barber-01 tenant created`);
