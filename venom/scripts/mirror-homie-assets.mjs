/**
 * FÁZE 1 — Mirror homietattoo.cz
 * WordPress, custom theme, WAF → Playwright scraping only
 */
...
const SLUG = 'homie';
const ORIGIN = 'https://homietattoo.cz';
const OUT = `public/clones/${SLUG}`;
const PAGES = [
  { slug: 'home',     url: '/' },
  { slug: 'piercing', url: '/piercing/' },
  { slug: 'faq',      url: '/faq/' },
];
...
fs.mkdirSync(`${OUT}/wp-content/themes/homietatto_wtw`, { recursive: true });
...
for (const p of PAGES) {
  ...
  page.on('response', async (resp) => {
    ...
    try {
      ...
      if (ct.includes('css') || rel.endsWith('.css')) {
        ...
        } else if (rel.includes('/wp-content/themes/')) {
          const subRel = rel.replace('/wp-content/themes/', '');
          fs.mkdirSync(`${OUT}/wp-content/themes/${path.dirname(subRel)}`, { recursive: true });
          const dest = `${OUT}/wp-content/themes/${subRel}`;
          if (!fs.existsSync(dest)) fs.writeFileSync(dest, body);
          imgMap.set(url, `/clones/${SLUG}/wp-content/themes/${subRel}`);
          imgMap.set(url.split('?')[0], `/clones/${SLUG}/wp-content/themes/${subRel}`);
        } else {
  ...
  try { await page.click('.cookie-law-info-bar a[id*="accept"], .cli-accept-all, #cookie_action_close_header', { timeout: 2000 }); } catch {}
...
function processHtml(html) {
  ...
  html = html.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|facebook\.net|bookio|cookielaw)[^>]*>[\s\S]*?<\/script>/gi, '');
  html = html.replace(/<noscript>[^<]*(?:googletagmanager|gtm)[^<]*<\/noscript>/gi, '');
  html = html.replace(/href="\/(?!clones|demo|_next)[^"]*"/g, 'href="#"');
  const linkMap = {
    '/piercing/': '/demo/homie-demo/piercing',
    '/faq/': '/demo/homie-demo/faq',
    '/': '/demo/homie-demo',
  };
...
for (const p of PAGES) {
  ...
  const extLeft = (processed.match(/https?:\/\/(?:www\.)?homietattoo\.cz/gi) || []).length;
  console.log(`${p.slug}: ${raw.length}→${processed.length} | homie refs: ${extLeft}`);
}
...
for (const fname of fs.readdirSync(`${OUT}/css`)) {
  ...
  css = css.replace(/url\((['"]?)https?:\/\/(?:www\.)?homietattoo\.cz([^'")]+)\1\)/gi,
