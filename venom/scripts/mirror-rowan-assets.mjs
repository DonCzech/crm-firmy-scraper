/**
 * Mirror rowan.legal — WordPress Bedrock (advokátní kancelář)
 * Run: node scripts/mirror-rowan-assets.mjs
 */
...
const SLUG   = 'rowan';
const ORIGIN = 'https://rowan.legal';
...
walk(`${OUT}/app`, (fp) => { if (fp.endsWith('.css')) fs.writeFileSync(fp, fixCss(fs.readFileSync(fp, 'utf8'))); });

const LINK_MAP = {
  '/':               `/demo/rowan-legal-demo`,
  '/o-nas/':         `/demo/rowan-legal-demo/o-nas`,
  '/o-nas':          `/demo/rowan-legal-demo/o-nas`,
  '/nase-sluzby/':   `/demo/rowan-legal-demo/sluzby`,
  '/nase-sluzby':    `/demo/rowan-legal-demo/sluzby`,
  '/kontakty/':      `/demo/rowan-legal-demo/kontakty`,
  '/kontakty':       `/demo/rowan-legal-demo/kontakty`,
  '/aktualne/':      `/demo/rowan-legal-demo`,
  '/aktualne':       `/demo/rowan-legal-demo`,
};

function processHtml(html) {
  for (const [orig, local] of assetMap) html = html.replaceAll(orig, local);
  html = html.replace(/(\/clones\/rowan\/[^"'?#\s]+)\?[^"'>\s]*/g, '$1');
  ...
  html = html.replace(/href="\/aktualne\/[^"]*"/gi, `href="/demo/rowan-legal-demo"`);
  html = html.replace(/href="\/nase-sluzby\/[^"]*"/gi, `href="/demo/rowan-legal-demo/sluzby"`);
  html = html.replace(/href="\/o-nas\/[^"]*"/gi, `href="/demo/rowan-legal-demo/o-nas"`);
  ...
  html = html.replace(/href="https?:\/\/(?:www\.)?rowan\.legal\/[^"]*"/gi, `href="/demo/rowan-legal-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?rowan\.legal"/gi, `href="/demo/rowan-legal-demo"`);

  html = html.replace(/ROWAN\s+LEGAL(?!\s*Demo)/g, 'ROWAN LEGAL Demo');
  html = html.replace(/Rowan\s+Legal(?!\s*Demo)/g, 'Rowan Legal Demo');
  html = html.replace(/rowan\.legal/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@rowan\.legal/gi, 'info@demo.local');
  ...
  // Fix Bedrock-style wp paths (/wp/wp-includes/ → /clones/rowan/wp/)
  html = html.replace(/(["'\s\(=,])\/wp\/wp-includes\//g, `$1/clones/${SLUG}/wp/wp-includes/`);
  html = html.replace(/(["'\s\(=,])\/wp\/wp-content\//g, `$1/clones/${SLUG}/wp/wp-content/`);
  html = html.replace(/(["'\s\(=,])\/app\/(themes|plugins|uploads)\//g, `$1/clones/${SLUG}/app/$2/`);
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/rowan\.legal/gi) || []).length;
