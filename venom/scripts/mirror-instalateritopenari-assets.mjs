/**
 * Mirror instalateritopenari.cz — instalatérství + topenářství Praha
 * Run: node -e "import('/Users/apple/DEV/CRM/venom/scripts/mirror-instalateritopenari-assets.mjs')"
 */
...
const ORIGIN = 'https://www.instalateritopenari.cz';
...
const LINK_MAP = {
  '/':            `/demo/instalateritopenari-demo`,
  '/sluzby/':     `/demo/instalateritopenari-demo/sluzby`,
  '/sluzby':      `/demo/instalateritopenari-demo/sluzby`,
  '/reference/':  `/demo/instalateritopenari-demo/reference`,
  '/reference':   `/demo/instalateritopenari-demo/reference`,
  '/kontakt/':    `/demo/instalateritopenari-demo/kontakt`,
  '/kontakt':     `/demo/instalateritopenari-demo/kontakt`,
  '/o-nas/':      `/demo/instalateritopenari-demo`,
  '/o-nas':       `/demo/instalateritopenari-demo`,
  '/galerie/':    `/demo/instalateritopenari-demo/reference`,
  '/cenik/':      `/demo/instalateritopenari-demo/sluzby`,
};
...
function processHtml(html) {
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="https?://(?:www\\.)?instalateritopenari\\.cz${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?instalateritopenari\.cz\/[^"]*"/gi, `href="/demo/instalateritopenari-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?instalateritopenari\.cz"/gi, `href="/demo/instalateritopenari-demo"`);
  ...
  // Brand scrub — careful not to match slug path
  html = html.replace(/Instalatéři\s*(?:a\s*)?Topenáři(?!\s*Demo)/g, 'Instalatéři a Topenáři Demo');
  html = html.replace(/instalateritopenari\.cz/gi, 'demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/instalateritopenari\.cz/gi) || []).length;
