...
const SLUG = 'maidenstudio';
const TENANT_SLUG = 'maidenstudio-demo';
const ACCESS_TOKEN = 'maiden' + Math.random().toString(36).slice(2, 10);
...
// Wix CSS files may not have .css extension (e.g. layoutCss)
...
function patchHtml(rawHtml) {
  ...
  const BAD_SRC = ['googletagmanager', 'google-analytics', 'facebook.net', 'fbevents',
    'clarity.ms', 'wix.com', 'cloudfront.net', 'doubleclick',
  ...
  // Rewrite wixstatic URLs to local
  body = rewriteWixImages(body);

  // Fix internal links
  body = body.replace(/https?:\/\/(?:www\.)?maidenstudio\.cz\/manikura/gi, '/demo/maidenstudio-demo/manikura');
  body = body.replace(/https?:\/\/(?:www\.)?maidenstudio\.cz\/o-nas/gi, '/demo/maidenstudio-demo/o-nas');
  body = body.replace(/https?:\/\/(?:www\.)?maidenstudio\.cz\/?(?=[\"'])/gi, '/demo/maidenstudio-demo');

  // Brand scrub
  body = body.replace(/Maiden Studio(?!\s*Demo)/gi, 'Maiden Studio Demo');
  body = body.replace(/maidenstudio\.cz/gi, 'demo.local');
  body = body.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  body = body.replace(/[a-z.]+@maidenstudio\.cz/gi, 'info@demo.local');
  body = body.replace(/href="https?:\/\/(?:www\.)?(?:facebook|instagram|youtube|tiktok)\.com\/[^"]*"/gi, 'href="#"');

  return KILL + '\n' + allStyles + '\n' + body;
}

log('=== Seed maidenstudio-demo ===');
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Maiden Studio', 'nehtové studio Praha',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'maidenstudio.cz', cms: 'Wix Thunderbolt' })]);
...
for (const p of PAGES) {
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo Maiden Studio`, 'Ukázka šablony pro nehtové studio Praha.']);
