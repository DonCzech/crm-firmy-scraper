/**
 * FÁZE 1 — mirror honeygrebovka.cz (Wix Thunderbolt)
 */
...
const ORIGIN = 'https://www.honeygrebovka.cz';
...
console.log('Navigating honeygrebovka.cz...');
await page.goto(ORIGIN, { waitUntil: 'networkidle', timeout: 45000 });
await page.waitForTimeout(4000);

// Dismiss cookie
try {
  await page.click('button:has-text("Accept"), button:has-text("Přijmout"), button:has-text("OK"), [data-testid="cookie-accept"]', { timeout: 3000 });
  console.log('Cookie dismissed');
} catch { console.log('No cookie'); }

// Scroll
await page.evaluate(async () => {
  for (let i = 0; i < 20; i++) { window.scrollBy(0, window.innerHeight); await new Promise(r => setTimeout(r, 300)); }
  window.scrollTo(0, 0);
});
...
// Also take origin screenshot
await page.screenshot({ path: '/tmp/honey-orig-full.png', fullPage: true });
...
console.log(`CMS: Wix=${rawLower.includes('wix')} parastorage=${rawLower.includes('parastorage')}`);
console.log(`ImgMap size: ${imgMap.size}`);
