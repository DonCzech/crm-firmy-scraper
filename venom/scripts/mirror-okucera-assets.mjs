/**
 * Mirror ondrejkucera.com — realitní makléř
 * Run: node scripts/mirror-okucera-assets.mjs
 */
...
const SLUG   = 'okucera';
const ORIGIN = 'https://ondrejkucera.com';
const OUT    = `public/clones/${SLUG}`;

const PAGES = [
  { slug: 'home',     url: '/' },
  { slug: 'o-mne',    url: '/o-mne' },
  { slug: 'jak-na-to', url: '/jak-na-to' },
...
function fixCss(c) {
  for (const [o,l] of assetMap) c = c.replaceAll(o,l);
  return c;
}
['assets','css','js','static','_next','dist','public','media'].forEach(d => walk(`${OUT}/${d}`, (fp) => { if (fp.endsWith('.css')) fs.writeFileSync(fp, fixCss(fs.readFileSync(fp, 'utf8'))); }));

const LINK_MAP = {
  '/':         `/demo/ondrej-kucera-demo`,
  '/o-mne':    `/demo/ondrej-kucera-demo/o-mne`,
  '/jak-na-to': `/demo/ondrej-kucera-demo/jak-na-to`,
  '/kontakt':  `/demo/ondrej-kucera-demo/kontakt`,
  '/nabidka-nemovitosti': `/demo/ondrej-kucera-demo`,
  '/predano':  `/demo/ondrej-kucera-demo`,
  '/clanky':   `/demo/ondrej-kucera-demo`,
  '/reference':`/demo/ondrej-kucera-demo`,
  '/serial-jak-pronajmout': `/demo/ondrej-kucera-demo`,
  '/jak-pronajmout': `/demo/ondrej-kucera-demo`,
};

function processHtml(html) {
  for (const [orig, local] of assetMap) html = html.replaceAll(orig, local);
  html = html.replace(/(\/clones\/okucera\/[^"'?#\s]+)\?[^"'>\s]*/g, '$1');
  ...
  // Strip article/clanek links → home
  html = html.replace(/href="\/clanek\/[^"]*"/gi, `href="/demo/ondrej-kucera-demo"`);
  html = html.replace(/href="\/nemovitost\/[^"]*"/gi, `href="/demo/ondrej-kucera-demo"`);
  ...
  html = html.replace(/href="https?:\/\/(?:www\.)?ondrejkucera\.com\/[^"]*"/gi, `href="/demo/ondrej-kucera-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?ondrejkucera\.com"/gi, `href="/demo/ondrej-kucera-demo"`);

  // Brand scrub
  html = html.replace(/Ondřej\s+Kučera(?!\s*Demo)/g, 'Ondřej Kučera Demo');
  html = html.replace(/ONDŘEJ\s+KUČERA(?!\s*DEMO)/g, 'ONDŘEJ KUČERA DEMO');
  html = html.replace(/ondrejkucera\.com/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@ondrejkucera\.com/gi, 'info@demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/ondrejkucera\.com/gi) || []).length;
