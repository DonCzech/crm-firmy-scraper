/**
 * FÁZE 2 — Seed DB pro autoservis-garant-demo
 * WordPress carservice theme (autoservis Praha), 4 stránky
 *
 * Spustit: node scripts/seed-garant-demo.mjs
 */
...
const SLUG = 'garant';
const TENANT_SLUG = 'autoservis-garant-demo';
const ACCESS_TOKEN = 'gar' + Math.random().toString(36).slice(2, 10);
...
// Load all autoptimize CSS bundles
const CSS_URLS = [];
const cssDir = `public/clones/${SLUG}/wp-content/cache/autoptimize/css`;
if (fs.existsSync(cssDir)) {
  for (const f of fs.readdirSync(cssDir)) {
    if (f.endsWith('.css')) CSS_URLS.push(`/clones/${SLUG}/wp-content/cache/autoptimize/css/${f}`);
  }
}
// Add theme custom CSS
if (fs.existsSync(`public/clones/${SLUG}/wp-content/themes/carservice/custom.css`)) {
  CSS_URLS.push(`/clones/${SLUG}/wp-content/themes/carservice/custom.css`);
}

const JS_URLS = []; // Skip — too complex
...
function patchHtml(rawHtml) {
  ...
  body = body.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|cmplz|cookiebot|analytics|trustindex|whatsapp|youtube|facebook)[^>]*>[\s\S]*?<\/script>/gi, '');
  body = body.replace(/<noscript><iframe[^>]*googletagmanager[^>]*><\/iframe><\/noscript>/gi, '');

  body = body.replace(/<div[^>]*(?:cmplz-cookiebanner|cookie-banner|cookie-consent|gdpr-banner)[^>]*>[\s\S]{0,8000}?<\/div>/gi, '');

  // Strip YouTube embeds → placeholder
  body = body.replace(/<iframe[^>]*(?:youtube\.com|youtu\.be)[^>]*>[\s\S]*?<\/iframe>/gi,
    '<div style="background:#000;aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;color:#666">Video</div>');
...
log('=== Seed autoservis-garant-demo ===');
...
`, [TENANT_SLUG, tpl.rows[0].id, 'Demo Autoservis GARANT', 'autoservis Praha',
    'info@demo.local', ACCESS_TOKEN,
    JSON.stringify({ cloneSource: 'autoservis-garant.cz', cms: 'WordPress + carservice theme' })]);
...
for (const p of PAGES) {
  ...
  const brand = (html.match(/autoservis-garant\.cz/gi) || []).length;
  ...
  `, [tid, p.slug, p.title, p.isHome,
      `${p.title} — Demo Autoservis GARANT`, 'Ukázka šablony pro autoservis v Praze.']);
