/**
 * Mirror stavbadesign.cz — WordPress Bedrock + Tailwind v4 + Vite
 * Run: node scripts/mirror-stavbadesign-assets.mjs
 */
...
const ORIGIN = 'https://www.stavbadesign.cz';
...
for (const p of PAGES) {
  ...
  page.on('response', async (resp) => {
    ...
    if (isJs && /gtm|analytics|pixel|hotjar|cookiebot|gtag|fbevents|seznam|bat\.bing/.test(base)) return;
    try {
      const dest = `${OUT}${rel}`;
  ...
  try { await page.click('button[class*="accept"], #CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll', { timeout: 2000 }); } catch {}
...
const BASE = 'https://www.stavbadesign.cz';
const LINK_MAP = {
  '/':            `/demo/stavbadesign-demo`,
  '/sluzby/':     `/demo/stavbadesign-demo/sluzby`,
  '/reference/':  `/demo/stavbadesign-demo/reference`,
  '/kontakty/':   `/demo/stavbadesign-demo/kontakty`,
  '/o-nas/':      `/demo/stavbadesign-demo`,
  '/novinky/':    `/demo/stavbadesign-demo`,
  '/akce/':       `/demo/stavbadesign-demo`,
};

function processHtml(html) {
  for (const [orig, local] of assetMap) html = html.replaceAll(orig, local);
  html = html.replace(/(\/clones\/stavbadesign\/[^"'?#\s]+)\?[^"'>\s]*/g, '$1');

  html = html.replace(/<script[^>]*src="https?:\/\/[^"]+"[^>]*>[\s\S]*?<\/script>/gi, '');
  html = html.replace(/<script[^>]*(?:googletagmanager|gtag|hotjar|cookiebot|analytics|fbevents|seznam|bat\.bing)[^>]*>[\s\S]*?<\/script>/gi, '');
  html = html.replace(/<noscript><iframe[^>]*googletagmanager[^>]*><\/iframe><\/noscript>/gi, '');
  html = html.replace(/<div[^>]*(?:cookie-banner|cookie-consent|gdpr|cookiebot|CybotCookiebot)[^>]*>[\s\S]{0,8000}?<\/div>/gi, '');
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="${BASE.replace(/\./g,'\\.')}${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?stavbadesign\.cz\/sluzby\/[^"]*"/gi, `href="/demo/stavbadesign-demo/sluzby"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?stavbadesign\.cz\/reference\/[^"]*"/gi, `href="/demo/stavbadesign-demo/reference"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?stavbadesign\.cz\/[^"]*"/gi, `href="/demo/stavbadesign-demo"`);

  html = html.replace(/Stavba\s*Design(?!\s*Demo)/g, 'Stavba Design Demo');
  html = html.replace(/stavbadesign\.cz/gi, 'demo.local');
  html = html.replace(/\+420\s*\d{3}\s*\d{3}\s*\d{3}/g, '+420 608 288 777');
  html = html.replace(/[a-z0-9._-]+@stavbadesign\.cz/gi, 'info@demo.local');
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/stavbadesign\.cz/gi) || []).length;
