/**
 * Mirror lesarb.cz — lesnické a arboristické práce
 * Run: node scripts/mirror-lesarb-assets.mjs (from venom/ dir)
 */
...
const SLUG   = 'lesarb';
const ORIGIN = 'https://lesarb.cz';
const OUT    = path.join(__dirname, '..', 'public', 'clones', SLUG);

const PAGES = [
  { slug: 'home',      url: '/' },
  { slug: 'sluzby',    url: '/kaceni-stromu/' },
  { slug: 'reference', url: '/pece-o-stromy/' },
...
for (const p of PAGES) {
  ...
  try { await page.click('#CybotCookiebotDialogBodyLevelButtonLevelOptinAllowAll, button[class*="accept"], .cmplz-accept, [data-cc="accept-all"]', { timeout: 2000 }); } catch {}
...
const LINK_MAP = {
  '/':                      `/demo/lesarb-demo`,
  '/pece-o-stromy/':        `/demo/lesarb-demo/sluzby`,
  '/pece-o-stromy':         `/demo/lesarb-demo/sluzby`,
  '/kaceni-stromu/':        `/demo/lesarb-demo/sluzby`,
  '/kaceni-stromu':         `/demo/lesarb-demo/sluzby`,
  '/udrzba-pozemku/':       `/demo/lesarb-demo/sluzby`,
  '/udrzba-pozemku':        `/demo/lesarb-demo/sluzby`,
  '/kontakt/':              `/demo/lesarb-demo/kontakt`,
  '/kontakt':               `/demo/lesarb-demo/kontakt`,
  '/o-nas/':                `/demo/lesarb-demo`,
  '/o-nas':                 `/demo/lesarb-demo`,
  '/poptavka/':             `/demo/lesarb-demo/kontakt`,
  '/poptavka':              `/demo/lesarb-demo/kontakt`,
  '/reference/':            `/demo/lesarb-demo/reference`,
  '/reference':             `/demo/lesarb-demo/reference`,
};
...
function processHtml(html) {
  ...
  for (const [from, to] of Object.entries(LINK_MAP)) {
    ...
    html = html.replace(new RegExp(`href="https?://(?:www\\.)?lesarb\\.cz${from.replace(/\//g,'\\/')}(?=["/?#])`, 'g'), `href="${to}`);
  }
  html = html.replace(/href="https?:\/\/(?:www\.)?lesarb\.cz\/[^"]*"/gi, `href="/demo/lesarb-demo"`);
  html = html.replace(/href="https?:\/\/(?:www\.)?lesarb\.cz"/gi, `href="/demo/lesarb-demo"`);
  html = html.replace(/https?:\/\/(?:www\.)?lesarb\.cz(\/[^"'\s>]+)/g, `/clones/${SLUG}$1`);
  ...
  // Brand scrub
  html = html.replace(/Lesarb(?!\s*Demo)/g, 'Lesarb Demo');
  html = html.replace(/LESARB(?!\s*DEMO)/g, 'LESARB DEMO');
  html = html.replace(/lesarb\.cz/gi, 'demo.local');
  ...
  html = html.replace(/\/clones\/Lesarb Demo\//g, `/clones/${SLUG}/`);
  html = html.replace(/(\/clones\/${SLUG}\/[^\s"']*?)demo\.local/g, `$1lesarb.cz`);
...
for (const p of PAGES) {
  ...
  const brandLeft = (processed.match(/lesarb\.cz/gi) || []).length;
