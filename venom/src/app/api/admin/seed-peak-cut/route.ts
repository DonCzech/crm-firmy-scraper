...
// POST /api/admin/seed-peak-cut
// Seeds a peak-cut demo tenant for preview/testing.
...
export async function POST(req: NextRequest) {
  ...
  const tenantSlug = (slug as string | undefined) ?? "peak-cut-demo";

  try {
    const result = await createDemoTenantFromTemplate({
      email: `${tenantSlug}@demo.local`,
      templateKey: "peak-cut",
      industry: "barber",
      slug: tenantSlug,
    });
