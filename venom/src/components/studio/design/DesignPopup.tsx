...
export function DesignPopup({ openId, onClose }: Props) {
  ...
  return createPortal(
    <div data-studio>
      ...
      >
        <Header label={found.label} onClose={onClose} />
        <div className="flex-1 overflow-y-auto vs-scroll px-3.5 py-2">
          <Panel />
        </div>
        <Footer onClose={onClose} prefixes={found.prefixes ?? []} panelLabel={found.label} />
      </div>
    </div>,
    document.body
  );
...
function Footer({ onClose, prefixes, panelLabel }: { onClose: () => void; prefixes: string[]; panelLabel: string }) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-t border-[var(--vs-border)] p-2">
      ...
      <button
        type="button"
        onClick={onClose}
        className="h-9 flex-1 rounded bg-[#2563eb] text-[12.5px] font-semibold text-white hover:bg-[#1d4ed8]"
      >
        Hotovo
      </button>
    </div>
