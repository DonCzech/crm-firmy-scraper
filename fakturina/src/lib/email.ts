...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:Arial,Helvetica,sans-serif">
  <div style="max-width:580px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08)">
    ...
    <div style="padding:32px">
      <h1 style="margin:0 0 16px;font-size:20px;font-weight:700;color:#1e293b">${escapeHtml(title)}</h1>
      ${content}
...
function metaBox(items: { label: string; value: string }[]) {
  const rows = items.map(({ label, value }) =>
    `<tr>
      <td style="padding:8px 14px;font-size:13px;color:#64748b;border-bottom:1px solid #f1f5f9">${escapeHtml(label)}</td>
      <td style="padding:8px 14px;font-size:13px;font-weight:600;color:#1e293b;text-align:right;border-bottom:1px solid #f1f5f9">${escapeHtml(value)}</td>
    </tr>`
  ).join("");
...
export async function sendInvoiceEmail(data: InvoiceEmailData): Promise<string | null> {
  const content = `
    <p style="margin:0 0 14px;font-size:14px;color:#334155;line-height:1.65">
      Dobrý den, ${escapeHtml(data.clientName)},<br><br>
      zasíláme Vám fakturu č. <strong>${escapeHtml(data.invoiceNumber)}</strong> od ${escapeHtml(data.supplierName)}.
    </p>
    ${data.note ? `<p style="margin:0 0 14px;font-size:14px;color:#334155;line-height:1.65">${escapeHtml(data.note)}</p>` : ""}
    ${metaBox([
...
export async function sendQuoteEmail(data: QuoteEmailData): Promise<string | null> {
  const content = `
    <p style="margin:0 0 14px;font-size:14px;color:#334155;line-height:1.65">
      Dobrý den, ${escapeHtml(data.clientName)},<br><br>
      zasíláme Vám cenovou nabídku č. <strong>${escapeHtml(data.quoteNumber)}</strong> od ${escapeHtml(data.supplierName)}.
    </p>
    ${data.note ? `<p style="margin:0 0 14px;font-size:14px;color:#334155;line-height:1.65">${escapeHtml(data.note)}</p>` : ""}
    ${metaBox([
...
export async function sendReminderEmail(data: ReminderEmailData): Promise<string | null> {
  ...
  const bodyText = escapeHtml(renderTemplate(data.template, vars));
  ...
  const content = `
    ...
    ${metaBox([
      ...
      { label: "Částka k úhradě", value: fmt(data.total, data.currency) },
      { label: "Datum splatnosti", value: data.dueDate },
      ...(isOverdue ? [{ label: "Dní po splatnosti", value: String(data.daysOverdue) }] : []),
    ])}
